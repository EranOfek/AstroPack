...
var
    ...
    _Addr: AnsiString;  //Chen
begin
    // Convert host address to IP address
    _Addr := AnsiString(FAddress);  //Chen 04/02/15
    FIPAddress := inet_addr(PAnsiChar(_Addr));  //Chen FAddress));
    if FIPAddress <> INADDR_NONE then
        ...
        Phe := GetHostByName(PAnsiChar(_Addr));  //Chen FAddress));
        ...
        FHostName  := String(Phe^.h_name);  //Chen 04/02/15
    end;

    FHostIP       := String( StrPas(inet_ntoa(TInAddr(FIPAddress))) );
    FAddrResolved := TRUE;
end;
...
begin
    ...
    try
        ...
        Msg := 'Pinging';  //Chen from Delphi code written by F. Piette';
