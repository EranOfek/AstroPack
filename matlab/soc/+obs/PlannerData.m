...
classdef PlannerData < dynamicprops
    ...
    %-------------------------------------------------------- 
    methods % Constructor            
        function Obj = PlannerData(varargin)           
            
            % Set DbQuery
            if numel(varargin) == 1
                Obj.Query = varargin{1};
                io.msgLog(LogLevel.Debug, ['PlannerData created, DbQuery: ', Obj.Query.Uuid]);
            else
                io.msgLog(LogLevel.Debug, 'PlannerData created');
            end
        ...
        % Destructor
        function delete(Obj)
            io.msgLog(LogLevel.Debug, 'PlannerData deleted');
        end
    end
