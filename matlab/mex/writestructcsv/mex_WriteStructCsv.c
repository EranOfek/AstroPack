    ...
    for (jstruct = 0;  jstruct < NStructElems;  jstruct++) {
        for (ifield=0;  ifield < nfields;  ifield++) {
            ...
            switch (classIDflags[ifield]):
                ...
                // double
                case mxDOUBLE_CLASS:
                    f_double = *mxGetDoubles(fld);
                    fprintf(fd, "%lf,", f_double);
                    break;

                // single
                case mxSINGLE_CLASS:
                    f_single = *mxGetSingles(fld);
                    fprintf(fd, "%f,", f_single);
                    break;

                case mxINT8_CLASS:
                    f_int = mxGetInt8s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxUINT8_CLASS:
                    f_uint = mxGetUint8s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxINT16_CLASS:
                    f_int = mxGetInt16s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxUINT16_CLASS:
                    f_uint = mxGetUint16s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxINT32_CLASS:
                    f_int = mxGetInt32s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxUINT32_CLASS:
                    f_uint = mxGetUint32s(fld);
                    fprintf(fd, "%d,", f_int);
                    break;

                case mxINT64_CLASS:
                    f_int64 = mxGetInt64s(fld);
                    fprintf(fd, "%d,", f_int64);
                    break;

                case mxUINT64_CLASS
                    f_uint64 = mxGetUint64s(fld);
                    fprintf(fd, "%d,", f_uint64);
                    break;
            }
