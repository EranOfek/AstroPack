function [PC_array, Result] = fitJointVisit(PC_array, AI_array, Args)
    ...
    % ----- Step 5: fitPar on the pooled data -------------------------------
    if Args.Verbose
        fprintf('Step 5: fitPar on pool (OptSeq=%s)\n', Args.OptSeqName);
    end
    % Load the joint OptSeq from name. We only need StageCatalog (OptSeq
    % definitions); FunCatalog isn't used here, so we don't have to pass
    % ZenithAngle/Temperature/Pressure — defaults are fine.
    [~, StageCat] = imUtil.calib.predefSeqCompositeFun();
    if ~isfield(StageCat, Args.OptSeqName)
        error('PhotCalibTrans:fitJointVisit:UnknownOptSeq', ...
              'OptSeqName "%s" not found in predefSeqCompositeFun StageCatalog.', ...
              Args.OptSeqName);
    end
    JointOptSeq = StageCat.(Args.OptSeqName);

    % CostArgs is the SINGLE cell forwarded to costFun by fitPar/fitMultiStage.
    % X, Y, SigmaClipMethod, OuterX are top-level args on fitPar, not in CostArgs.
    CostArgs = { ...
    ...
    % Pluck SigmaClipMethod and a couple of other args out of CalibArgs if
    % the user passed them at the wrapper level.
    SigmaClipMethod = pickArg(Args.CalibArgs, 'SigmaClipMethod', 'median');

    [JointCF, FitResult] = JointCF.fitPar( ...
        JP.TransWvl, JP.Flux, ...
        'X',                  JP.XFULL, ...
        'Y',                  JP.YFULL, ...
        'CostArgs',           CostArgs, ...
        'OptimizationSequence', JointOptSeq, ...
    ...
    if Args.Verbose
        % FitResult is a struct ARRAY with one entry per stage. Use the
        % last stage for the summary line.
        LastStage = FitResult(end);
        fprintf('=== fitJointVisit done: NCalUsed=%d, RMS=%.4f mag ===\n', ...
                LastStage.NCalUsed, LastStage.RMS);
    end
end
