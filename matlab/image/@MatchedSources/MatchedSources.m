...
%                 Result(I).Nobs     = sum(~isnan(Data), 1);
    ...
    methods % find sources
        function FlagOrphan = findOrphans(Obj, Args)
            ...
            %                   Default is 1.
            %            'FieldName' - Field name in which to look for NaNs
            ...
            % Example: MS = MatchedSources;
            %          MS.addMatrix(rand(100,200).*10,'MAG');
            %          F = MS.findOrphans('FieldName','MAG')
            
            arguments
                Obj
                Args.MaxNepochs  = 1;
                Args.FieldName   = MatchedSources.DefNamesX;
            end
            
            Nobj = numel(Obj);
            for Iobj=1:1:Nobj
                Matrix = getMatrix(Obj, Args.FieldName);
                % number of epochs in which each source was detected
                NepochPerSrc = sum(~isnan(Matrix), 1);
