function [BestCen,BestRad,BestRMS,BestDif,BestRDi,Info]=circ_fit(Data,Geom,MinPar,Nstep)
...
switch MinPar
 case 'rms'
    ...
    [Min,MinInd] = min(RMS(:));
    [I1,I2]      = imUtil.image.ind2sub_fast(size(RMS),MinInd);
    MinInd       = [I1, I2];
    %[Min,MinInd] = min2d(RMS);
 case 'diff' 
    Diff         = max(Dist,[],3) - min(Dist,[],3);
    [Min,MinInd] = min(Diff(:));
    [I1,I2]      = imUtil.image.ind2sub_fast(size(Diff),MinInd);
    MinInd       = [I1, I2];
 ...
 case 'rd'
    ...
    [Min,MinInd] = min(RD(:));
    [I1,I2]      = imUtil.image.ind2sub_fast(size(RD),MinInd);
    MinInd       = [I1, I2];
    %[Min,MinInd] = Util.stat.minnd(RD);
 otherwise
