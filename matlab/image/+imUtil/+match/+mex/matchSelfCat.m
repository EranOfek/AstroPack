% Match a catalog of [RA, Dec] coordinates to itself (excluding searched object), using a radius search
% Description: Match sources in catalog against itself (on the sphere).
...
% Input  : - Vector of RA of catalog. Must be sorted by Dec.
...
% Compilation: mex -O CXXFLAGS='$CXXFLAGS -fopenmp -std=c++17' LDFLAGS='$LDFLAGS -fopenmp' matchSelfCat.cpp
% Example: 
%{
            ...
            [IndNearest2to1, DistNearest, Nmatch] = ...
                      imUtil.match.mex.matchSelfCat(RA1, Dec1, 1e-3);
            [IndNearest2to1, DistNearest, Nmatch, IndAll] = ...
                      imUtil.match.mex.matchSelfCat(RA1, Dec1, 1e-3, false, false, true);
%}