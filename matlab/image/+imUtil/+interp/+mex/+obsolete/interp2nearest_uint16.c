...
void nearestInterp2D(
    const double *X, const double *Y, const uint16_T *values,
    const double *outX, const double *outY,
    uint16_T *output, mwSize nRows, mwSize nCols, mwSize outRows, mwSize outCols)
{
    ...
    for (mwSize j = 0; j < outCols; j++) {
        for (mwSize i = 0; i < outRows; i++) {
            ...
            if (x < xMin || x > xMax || y < yMin || y > yMax) {
                output[i + j * outRows] = 0;  // Assign 0 for out-of-bound points
            } else {
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    uint16_T *values = (uint16_T *)mxGetData(prhs[2]);
    ...
    plhs[0] = mxCreateNumericMatrix(outRows, outCols, mxUINT16_CLASS, mxREAL);
