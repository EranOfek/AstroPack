...
// mex -largeArrayDims CFLAGS="\$CFLAGS -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp" nearestInterp2D_nan_fast.c
/* The nearest-neighbor interpolation function */
void nearestInterp2D(
    const double *X, const double *Y, const double *values,
    const double *outX, const double *outY,
    double *output, mwSize nRows, mwSize nCols, mwSize outRows, mwSize outCols)
{
