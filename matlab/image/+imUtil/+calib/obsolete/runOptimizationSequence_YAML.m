function [SequenceResults, FinalCalibData, FinalFieldParams] = runOptimizationSequence_YAML(CalibData, TransFun, YAMLConfig, Args)
    % Execute multi-stage optimization sequence defined in YAML configuration
    % Input  : - CalibData - Structure with calibrator data (from findCalibrators) with fields:
    ...
    %          - YAMLConfig - YAML configuration structure (from yaml.ReadYaml) or
    %                   path to YAML file. Must contain OptimizationSequence field.
    %          * ...,key,val,...
    %            'Verbose' - Enable verbose output.
    %                   Default is true.
    % Output : - SequenceResults - Cell array with results from each optimization stage.
    ...
    % Example: [Results, CalibData, FieldParams] = imUtil.calib.runOptimizationSequence_YAML(...
    %                              CalibData, TransFun, '~/config/CalibPhotAB.yml');
    ...
    for iStage = 1:NumStages
        ...
        if strcmp(Method, 'nonlinear')
            ...
            [OptParams, Fval, ExitFlag, Output, ResultData] = ...
                imUtil.calib.minimizeNonlinear(CurrentCalibData, CurrentParamValues, TransFun, StageStruct, ...
                    'FieldParams', CurrentFieldParams, 'Verbose', Args.Verbose);

        elseif strcmp(Method, 'linear')
            % Linear minimizer
            [OptParams, Fval, ExitFlag, Output, ResultData] = ...
                imUtil.calib.minimizeLinear(CurrentCalibData, CurrentParamValues, TransFun, StageStruct, ...
                    'FieldParams', CurrentFieldParams, 'Verbose', Args.Verbose);
        else
