function [OptimalParams, Fval, ExitFlag, Output, ResultData] = minimizeNonlinear_DRAFT(CalibData, ParamValues, TransFun, Stage, Args)
    % Nonlinear optimization wrapper using lsqNonLinWithFixed
    % Optimizes transmission parameters for photometric calibration using
    % least-squares fitting with support for fixed parameters and sigma clipping.
    ...
    %            'OptimOptions' - optimoptions for lsqnonlin. Default is [].
    %
    ...
    %          Stage = struct('Name', "Normalization", ...
    %                         'FreeParams', [1], ...
    %                         'SigmaClipping', true, ...
    %                         'SigmaThreshold', 3.0, ...
    %                         'SigmaIterations', 3);
    %          [OptParams, Fval] = imUtil.calib.minimizeNonlinear_DRAFT(...
...
function [OptimizedPar, Cov, Info, ExitFlag, Output] = ...
    ...
    function DiffMag = getDiffMag(CalibData, P, TransFun, FieldParams)
        [~, ~, DiffMag, ~] = imUtil.calib.calculateCostFunction_DRAFT(...
    ...
    if Verbose
        [~, ~, TestDiffMag, ~] = imUtil.calib.calculateCostFunction_DRAFT(...
    ...
    try
        [OptimizedPar, Cov, Info] = tools.math.fit.lsqNonLinWithFixed(...
            X, Y, Sigma, Model, ...
            'InitPar', InitPar, ...
            'FitPar', FitMask, ...
            'Lb', Lb, ...
