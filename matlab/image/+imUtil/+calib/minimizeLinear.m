function [OptimalParams, Fval, ExitFlag, Output, ResultData] = minimizeLinear(CalibData, ParamValues, TransFun, Stage, Args)
    ...
    % Example: Stage = struct('Name', "FieldCorrection", 'FreeParams', [], ...,
    %                         'SigmaClipping', true, 'SigmaThreshold', 2.0, ...,
    %                         'SigmaIterations', 3);
    %          [OptParams, Fval] = imUtil.calib.minimizeLinear(...
...
function [OptimizedFieldParams, Cov, Info, ExitFlag, Output] = ...
    ...
    [~, ~, BaseDiffMag] = imUtil.calib.calculateCostFunction(...
    ...
    try
        ...
        % NOTE: fieldCorrectionLAST.m now uses linear kxy (not kxy²), allowing negative values
...
function A = buildChebyshevDesignMatrix(Coords)
    ...
    %
    % Column order matches fieldCorrectionLAST.m parameters:
    ...
    %
    % NOTE: fieldCorrectionLAST.m now applies kxy * T1(x)*T1(y) (linear formulation).
