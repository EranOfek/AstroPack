function [FunCatalog, StageCatalog] = predefSeqCompositeFun(Args)
    ...
    arguments
        ...
        % --- Optional AIRMASS-conditioned initialisation ---
        % When finite, replaces the constant Args.PWV_cm / .TauAod500 /
        % .QE_Center_Ang defaults with polynomial values from
        % astro.transmission.atmParFromAirmass. Coefficients are median
        % LAST fits vs AIRMASS across ~835 visits (June 2026). Gives
        % lsqnonlin a better initial guess for atmospheric parameters
        % than the flat class defaults, especially at high airmass
        % where median PWV rises steeply.
        %   NaN (default) - use the constant Args.* defaults above.
        %   finite AM     - override PWV_cm / TauAod500 / QE_Center_Ang.
        %                   Norm is not overridden - it is always fitted.
        % Callers that explicitly passed one of the atmospheric args do
        % NOT get overridden (the passed value wins). Detection uses the
        % arguments-block "value-changed-from-default" idiom.
        Args.InitFromAirmass (1,1) double = NaN
    end

    % --- AIRMASS-conditioned override -------------------------------------
    % Applied BEFORE the FunCatalog builder blocks so the polynomial values
    % flow through into Args.* consumers below. Only overrides args the
    % caller left at their class default (so an explicit user value wins).
    if isfinite(Args.InitFromAirmass)
        [PWV_pred, AOD_pred, Center_pred, ~] = ...
            astro.transmission.atmParFromAirmass(Args.InitFromAirmass);
        if Args.PWV_cm       == 1.4;      Args.PWV_cm       = PWV_pred;    end
        if Args.TauAod500    == 0.084;    Args.TauAod500    = AOD_pred;    end
        if Args.QE_Center_Ang == 5709.73; Args.QE_Center_Ang = Center_pred; end
    end
    ...
    % Gray transmission - only Normalization (no wavelength-dependent
    % component). Meant for diagnostic runs where you want to isolate
    % how much of the residual is explained by spectral modeling of
    % the atmosphere + QE vs by the position-dependent Tran2D surface.
    % Pair with StageCatalog.GrayJointFC (single JOINT_FC linear LS
    % over Tran2D coefficients only).
    FunCatalog.GrayFunList = FunCatalog.Normalization;
    ...
    % LAST_Joint_2Iter now lives as a RECIPE — a scalar struct with
    % .Stages (the 2-stage struct array), .NumRepeats (2), and
    % .IterOverrides (per-iter weighting + post-iter clip policy).
    % CompositeFun.fitMultiStage recognises Recipe-shaped OptSeqs and runs
    % NumRepeats iterations with the per-iter overrides applied. Passing a
    % Recipe alongside the legacy OuterMaxIter / OuterSigmaClip /
    % WeightedOuterIters args is a hard error.
    LAST_Joint_2Iter_Stages = struct( ...
        ...
        'Tran2DSplit',  {'none', ''}, ...
        'Description',  {'Pure Tran2D linear fit (10 cheby1_4_xt coefs); Norm untouched, kx0 absorbs the field-averaged zero-point (no explicit Norm/kx0 split)', ...
                         'Optimize QE center, aerosol AOD500, and water PWV (single lsqnonlin)'});
    LAST_Joint_2Iter_Overrides(1) = struct( ...
        'WeightingMode',       'none', ...
        'PostIterClipSigma',   3.0, ...
        'PostIterClipMethod',  'median', ...
        'PostIterClipStdFunc', 'std', ...
        'StageOverrides',      []);
    LAST_Joint_2Iter_Overrides(2) = struct( ...
    ...
    StageCatalog.LAST_Joint_2Iter = struct( ...
        'Stages',         LAST_Joint_2Iter_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'Joint 2-iter: unweighted -> 3sigma signed clip -> weighted (combined).');
    ...
    % Use with fitPar args '1Clip', true, 'OuterStdFunc', 'mad_std'.
    ...
    % ---- Single-step nonlinear OptSeqs -----------------------------------
    % Two minimal sequences that consist of exactly one nonlinear stage.
    % Useful as a quick-look alternative to the full multi-stage LAST
    % recipes when a single joint nonlinear fit over a small parameter
    % set is enough (e.g. for diagnostics, or when Tran2D is left
    % untouched and only a global transmission tweak is wanted).

    % One step: Norm + QE_Center (this is stage 2 of LAST_NormLin,
    % exposed as its own OptSeq for standalone use).
    StageCatalog.NormAndCenter_Nonlin = struct();
    StageCatalog.NormAndCenter_Nonlin.StageName      = 'NormAndCenter';
    StageCatalog.NormAndCenter_Nonlin.Method         = 'nonlinear';
    StageCatalog.NormAndCenter_Nonlin.FreeParams     = struct( ...
        'Function',  {'Normalization', 'QE_SkewedGaussian'}, ...
        'Parameter', {'Norm',          'Center_Ang'});
    StageCatalog.NormAndCenter_Nonlin.SigmaClip      = false;
    StageCatalog.NormAndCenter_Nonlin.SigmaThresh    = 3.0;
    StageCatalog.NormAndCenter_Nonlin.SigmaIter      = 0;
    StageCatalog.NormAndCenter_Nonlin.MinCalibrators = 30;
    StageCatalog.NormAndCenter_Nonlin.Description    = ...
        'Single-step nonlinear lsqnonlin over Norm + QE Center_Ang';

    % One step: Norm + QE_Center + Water PWV + Aerosol TauAod500,
    % all four fitted jointly by a single lsqnonlin call.
    StageCatalog.AllAtmos_Nonlin = struct();
    StageCatalog.AllAtmos_Nonlin.StageName      = 'NormCenterAtmos';
    StageCatalog.AllAtmos_Nonlin.Method         = 'nonlinear';
    StageCatalog.AllAtmos_Nonlin.FreeParams     = struct( ...
        'Function',  {'Normalization', 'QE_SkewedGaussian', 'Water',  'Aerosol'}, ...
        'Parameter', {'Norm',          'Center_Ang',        'PWV_cm', 'TauAod500'});
    StageCatalog.AllAtmos_Nonlin.SigmaClip      = false;
    StageCatalog.AllAtmos_Nonlin.SigmaThresh    = 3.0;
    StageCatalog.AllAtmos_Nonlin.SigmaIter      = 0;
    StageCatalog.AllAtmos_Nonlin.MinCalibrators = 30;
    StageCatalog.AllAtmos_Nonlin.Description    = ...
        'Single-step nonlinear lsqnonlin over Norm + QE Center_Ang + Water PWV_cm + Aerosol TauAod500';

    % Gray-transmission single-stage OptSeq. Pair with
    % FunCatalog.GrayFunList (Normalization only) - the model then has
    % no wavelength-dependent components, and this stage fits ONLY the
    % 10 Tran2D polynomial coefficients on the residuals of that gray
    % model. Useful as a baseline for how much of the fit's RMS
    % improvement is due to spectral modeling (Rayleigh + Water + Aerosol
    % + UMG + QE) vs pure position-dependent Tran2D correction.
    % Tran2DSplit='none' matches LAST_Joint_2Iter's stage 1 semantics:
    % Norm is held at 1, kx0 absorbs the field-averaged ZP.
    % Recipe-wrapped so it routes through fitMultiStage's JOINT_FC
    % handler (single-stage fitPar path doesn't recognise the
    % 'JOINT_FC' sentinel and would fall through to a plain lsqnonlin
    % nonlinear fit over Norm + Tran2D, defeating the point).
    GrayJointFC_Stage = struct( ...
        'StageName',      'GrayNormAndTran2D_Linear', ...
        'Method',         'linear', ...
        'FreeParams',     'JOINT_FC', ...
        'SigmaClip',      false, ...
        'SigmaThresh',    3.0, ...
        ...
        'MinCalibrators', 30, ...
        'Tran2DSplit',    'none', ...
        'Description',    'Gray-transmission JOINT_FC (Norm held at 1, Tran2D LS)');
    StageCatalog.GrayJointFC = struct( ...
        'Stages',         GrayJointFC_Stage, ...
        'NumRepeats',     1, ...
        'IterOverrides',  [], ...
        'Description',    ['JOINT_FC linear LS over Tran2D coefficients ', ...
                           'on GRAY (wavelength-independent) transmission']);
