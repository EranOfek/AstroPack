function [FunCatalog, StageCatalog] = predefSeqCompositeFun(Args)
    ...
    % Triangle transmission - synthetic piecewise-linear T(lambda):
    % 0 at 3000 A, 1 at 5700 A, 0 at 11000 A. Not physics; a shape
    % probe. Pairs with StageCatalog.TriangleJointFC to see how much
    % of the LAST_Joint_2Iter improvement comes from the SHAPE of
    % transmission (not the specific atmospheric components).
    FunCatalog.TriangleT = struct();
    FunCatalog.TriangleT.Name       = 'TriangleT';
    FunCatalog.TriangleT.Handle     = '@astro.transmission.triangleTransmission';
    FunCatalog.TriangleT.HandleType = 'named';
    FunCatalog.TriangleT.Params     = [5700, 3000, 11000];   % [Peak, Left, Right]
    FunCatalog.TriangleT.FitPar     = [false, false, false]; % edges + peak held fixed
    FunCatalog.TriangleT.ParamInfo  = struct( ...
        'Name',        {1, 2, 3}, ...
        'Description', {'PeakLambda_Ang', 'LeftEdge_Ang', 'RightEdge_Ang'}, ...
        'Min',         {3000, 3000, 5700}, ...
        'Max',         {11000, 5700, 11000});

    % Triangle-transmission FunList: Norm * Triangle(lambda). Two
    % functions total; Norm is the only fittable parameter.
    FunCatalog.TriangleFunList = [FunCatalog.Normalization, FunCatalog.TriangleT];
    ...
    LAST_Joint_2Iter_Stages = struct( ...
        'StageName',    {'NormAndTran2D_Linear', 'NormCenterAerosolWater'}, ...
        'Method',       {'linear', 'nonlinear'}, ...
        'FreeParams',   {'JOINT_FC', ...
                         struct('Function',  {'Normalization', 'QE_SkewedGaussian', 'Aerosol',   'Water'}, ...
                                'Parameter', {'Norm',          'Center_Ang',        'TauAod500', 'PWV_cm'})}, ...
        'SigmaClip',    {false, false}, ...
        'SigmaThresh',  {3.0, 3.0}, ...
        'SigmaIter',    {0, 0}, ...
        'MinCalibrators', {30, 30}, ...
        'Tran2DSplit',  {'none', ''}, ...
        'PostFitAbsorbNormToKx0', {false, false}, ...
        'KeepNormOnEntry', {false, false}, ...
        'Description',  {'Pure Tran2D linear fit (10 cheby1_4_xt coefs); Norm untouched, kx0 absorbs the field-averaged zero-point (no explicit Norm/kx0 split)', ...
                         'Optimize Norm + QE center + aerosol AOD500 + water PWV (single lsqnonlin)'});
    ...
    % Single-iteration variant of LAST_Joint_2Iter: same 2-stage sequence
    % (JOINT_FC linear + NormCenterAerosolWater nonlinear), but only one
    % outer iteration. No post-iter 3-sigma clip + re-fit. Same weighting
    % policy as iter 1 of the 2-iter recipe (unweighted). Useful as a
    % "no outer clip" baseline against LAST_Joint_2Iter to see how much
    % of the RMS improvement comes from the outlier-rejection pass.
    % Stage 2 of the 1-iter variant redistributes the fitted Norm into
    % kx0 (analytical DC shift, prediction preserved). Preserves the
    % "Norm+Tran2D degeneracy resolved at kx0" invariant end-to-end in
    % single-iter mode, where there is no outer-iter-2 stage-1 refit to
    % clean up the split otherwise.
    LAST_Joint_1Iter_Stages = LAST_Joint_2Iter_Stages;
    LAST_Joint_1Iter_Stages(2).PostFitAbsorbNormToKx0 = true;
    StageCatalog.LAST_Joint_1Iter = struct( ...
        'Stages',         LAST_Joint_1Iter_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Joint 1-iter (unweighted, no outer clip; Norm redistributed into kx0 after stage 2)');

    % Reversed-order variant: nonlinear atm+Norm+Center BEFORE
    % JOINT_FC Tran2D fit. Stage 1 (nonlinear) sees raw residuals
    % including the huge position-dependent DC and fits Norm/Center/AOD/
    % PWV. Stage 2 (JOINT_FC) PRESERVES stage 1's Norm (via
    % KeepNormOnEntry=true) and fits Tran2D coefficients on residuals
    % that assume that Norm. So stage 2 refines the position-dependent
    % correction ON TOP OF the atm-fitted throughput. Useful for
    % comparing "atm first" vs "Tran2D first" as diagnostics.
    LAST_Joint_1Iter_AtmosFirst_Stages = LAST_Joint_1Iter_Stages([2, 1]);
    LAST_Joint_1Iter_AtmosFirst_Stages(1).PostFitAbsorbNormToKx0 = false;  % no redistribution; stage 2 will use the Norm
    LAST_Joint_1Iter_AtmosFirst_Stages(2).KeepNormOnEntry        = true;   % stage 2 must not reset Norm=1
    StageCatalog.LAST_Joint_1Iter_AtmosFirst = struct( ...
        'Stages',         LAST_Joint_1Iter_AtmosFirst_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Joint 1-iter, atm-first: nonlinear (Norm+Center+AOD+PWV) -> JOINT_FC keeps that Norm, fits Tran2D');

    % ---- 3-stage sequential-atm variants ------------------------------
    % Instead of one 4-param nonlinear stage, break atmospheric fitting
    % into three 2-param nonlinear sub-stages: (Norm+Center), (Norm+AOD),
    % (Norm+PWV). Each sub-stage lets Norm slide alongside the specific
    % atm parameter it fits, so the fit walks along a lower-dimensional
    % surface at each step. Helps avoid the flat-ridge minima that a
    % joint 4-param fit runs into (PWV railing at 10 cm etc.).
    NormCenter_FP  = struct('Function',  {'Normalization', 'QE_SkewedGaussian'}, ...
                            'Parameter', {'Norm',          'Center_Ang'});
    NormAerosol_FP = struct('Function',  {'Normalization', 'Aerosol'}, ...
                            'Parameter', {'Norm',          'TauAod500'});
    NormWater_FP   = struct('Function',  {'Normalization', 'Water'}, ...
                            'Parameter', {'Norm',          'PWV_cm'});
    AtmSplit3_Stages = struct( ...
        'StageName',    {'NormCenter', 'NormAerosol', 'NormWater'}, ...
        'Method',       {'nonlinear',  'nonlinear',   'nonlinear'}, ...
        'FreeParams',   {NormCenter_FP, NormAerosol_FP, NormWater_FP}, ...
        'SigmaClip',    {false, false, false}, ...
        'SigmaThresh',  {3.0, 3.0, 3.0}, ...
        'SigmaIter',    {0, 0, 0}, ...
        'MinCalibrators', {30, 30, 30}, ...
        'Tran2DSplit',  {'', '', ''}, ...
        'PostFitAbsorbNormToKx0', {false, false, false}, ...
        'KeepNormOnEntry', {false, false, false}, ...
        'Description',  {'Nonlinear Norm + QE_Center only', ...
                         'Nonlinear Norm + Aerosol TauAod500 only', ...
                         'Nonlinear Norm + Water PWV_cm only'});

    % Variant 1: JOINT_FC first (Norm=1 held, kx0 absorbs DC), then the
    % three sequential (Norm+atm) sub-stages. Redistribute Norm into
    % kx0 after the final water sub-stage so the invariant is preserved
    % end-to-end.
    LAST_Joint_1Iter_Split3_Stages = [LAST_Joint_1Iter_Stages(1), AtmSplit3_Stages];
    LAST_Joint_1Iter_Split3_Stages(end).PostFitAbsorbNormToKx0 = true;
    StageCatalog.LAST_Joint_1Iter_Split3 = struct( ...
        'Stages',         LAST_Joint_1Iter_Split3_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Joint 1-iter, split atm: JOINT_FC -> (Norm+Center) -> (Norm+AOD) -> (Norm+PWV)');

    % Variant 2: Three sequential (Norm+atm) sub-stages FIRST, then
    % JOINT_FC preserving Norm from stage 3 - fits Tran2D on top of the
    % atm-adjusted throughput.
    JointFCKeepNorm_Stage = LAST_Joint_1Iter_Stages(1);
    JointFCKeepNorm_Stage.KeepNormOnEntry = true;
    LAST_Joint_1Iter_AtmosFirst_Split3_Stages = [AtmSplit3_Stages, JointFCKeepNorm_Stage];
    StageCatalog.LAST_Joint_1Iter_AtmosFirst_Split3 = struct( ...
        'Stages',         LAST_Joint_1Iter_AtmosFirst_Split3_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Joint 1-iter atm-first, split atm: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV) -> JOINT_FC keeps Norm');

    % 2-iter counterparts of the two _Split3 recipes. Same stage sequences,
    % but the outer loop runs twice with LAST_Joint_2Iter_Overrides: iter 1
    % unweighted + 3sigma signed clip on the residuals, iter 2 refit on the
    % clipped survivors with 'combined' weights. Same policy as the classic
    % LAST_Joint_2Iter, just applied to the Split3 stage sequence.
    StageCatalog.LAST_Joint_2Iter_Split3 = struct( ...
        'Stages',         LAST_Joint_1Iter_Split3_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'Joint 2-iter, split atm: JOINT_FC -> (Norm+Center) -> (Norm+AOD) -> (Norm+PWV), x2 with iter 1 unweighted + 3sigma clip and iter 2 combined-weighted');
    StageCatalog.LAST_Joint_2Iter_AtmosFirst_Split3 = struct( ...
        'Stages',         LAST_Joint_1Iter_AtmosFirst_Split3_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'Joint 2-iter atm-first, split atm: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV) -> JOINT_FC keeps Norm, x2 with iter 1 unweighted + 3sigma clip and iter 2 combined-weighted');

    % No-Tran2D counterparts of the two _Split3 recipes. Both contain
    % the three sequential (Norm+atm) sub-stages; the JOINT_FC stage is
    % simply omitted. Pair with 'UseTran2D', false.
    StageCatalog.LAST_Joint_1Iter_Split3_NoTran2D = struct( ...
        'Stages',         AtmSplit3_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Split3 without Tran2D: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV)');
    StageCatalog.LAST_Joint_1Iter_AtmosFirst_Split3_NoTran2D = struct( ...
        'Stages',         AtmSplit3_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'AtmosFirst_Split3 without Tran2D: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV)');

    % 2-iter counterparts of the NoTran2D recipes: same three atmosphere
    % sub-stages, outer loop runs twice with LAST_Joint_2Iter_Overrides.
    % Iter 1 unweighted + 3-sigma signed clip on residuals, iter 2 refit on
    % the survivors with 'combined' weights. Kept name-symmetric with the
    % Tran2D-enabled _2Iter_Split3 pair so callers can swap only the recipe
    % name (and UseTran2D=false) for a like-for-like no-Tran2D comparison.
    % Without a JOINT_FC stage the AtmosFirst and non-AtmosFirst variants
    % are literally the same stage sequence; both names are kept for API
    % symmetry with the Tran2D-enabled recipes.
    StageCatalog.LAST_Joint_2Iter_Split3_NoTran2D = struct( ...
        'Stages',         AtmSplit3_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'Split3 without Tran2D, 2-iter: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV), x2 with iter 1 unweighted + 3sigma clip and iter 2 combined-weighted');
    StageCatalog.LAST_Joint_2Iter_AtmosFirst_Split3_NoTran2D = struct( ...
        'Stages',         AtmSplit3_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'AtmosFirst_Split3 without Tran2D, 2-iter: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV), x2 with iter 1 unweighted + 3sigma clip and iter 2 combined-weighted');

    % ---- AWC-ordered variants: (Norm+AOD) -> (Norm+Water) -> (Norm+Center)
    % Same three sub-stages as the default CAW order, permuted [2, 3, 1].
    AtmSplit3_AWC_Stages = AtmSplit3_Stages([2, 3, 1]);

    LAST_Joint_1Iter_Split3_AWC_Stages = [LAST_Joint_1Iter_Stages(1), AtmSplit3_AWC_Stages];
    LAST_Joint_1Iter_Split3_AWC_Stages(end).PostFitAbsorbNormToKx0 = true;
    StageCatalog.LAST_Joint_1Iter_Split3_AWC = struct( ...
        'Stages',         LAST_Joint_1Iter_Split3_AWC_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Split3 AWC order: JOINT_FC -> (Norm+AOD) -> (Norm+PWV) -> (Norm+Center)');

    LAST_Joint_1Iter_AtmosFirst_Split3_AWC_Stages = [AtmSplit3_AWC_Stages, JointFCKeepNorm_Stage];
    StageCatalog.LAST_Joint_1Iter_AtmosFirst_Split3_AWC = struct( ...
        'Stages',         LAST_Joint_1Iter_AtmosFirst_Split3_AWC_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'AtmosFirst_Split3 AWC order: (Norm+AOD) -> (Norm+PWV) -> (Norm+Center) -> JOINT_FC keeps Norm');

    StageCatalog.LAST_Joint_1Iter_Split3_AWC_NoTran2D = struct( ...
        'Stages',         AtmSplit3_AWC_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Split3 AWC order without Tran2D: (Norm+AOD) -> (Norm+PWV) -> (Norm+Center)');

    % WCA order: JOINT_FC -> (Norm+Water) -> (Norm+Center) -> (Norm+AOD).
    % Base AtmSplit3_Stages is CAW=[C,A,W]=[1,2,3]; WCA = [3, 1, 2].
    AtmSplit3_WCA_Stages = AtmSplit3_Stages([3, 1, 2]);
    LAST_Joint_1Iter_Split3_WCA_Stages = [LAST_Joint_1Iter_Stages(1), AtmSplit3_WCA_Stages];
    LAST_Joint_1Iter_Split3_WCA_Stages(end).PostFitAbsorbNormToKx0 = true;
    StageCatalog.LAST_Joint_1Iter_Split3_WCA = struct( ...
        'Stages',         LAST_Joint_1Iter_Split3_WCA_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Split3 WCA order: JOINT_FC -> (Norm+PWV) -> (Norm+Center) -> (Norm+AOD)');

    % ACW order: JOINT_FC -> (Norm+AOD) -> (Norm+Center) -> (Norm+Water).
    % Base AtmSplit3_Stages is CAW=[C,A,W]=[1,2,3]; ACW = [2, 1, 3].
    AtmSplit3_ACW_Stages = AtmSplit3_Stages([2, 1, 3]);
    LAST_Joint_1Iter_Split3_ACW_Stages = [LAST_Joint_1Iter_Stages(1), AtmSplit3_ACW_Stages];
    LAST_Joint_1Iter_Split3_ACW_Stages(end).PostFitAbsorbNormToKx0 = true;
    StageCatalog.LAST_Joint_1Iter_Split3_ACW = struct( ...
        'Stages',         LAST_Joint_1Iter_Split3_ACW_Stages, ...
        'NumRepeats',     1, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides(1), ...
        'Description',    'Split3 ACW order: JOINT_FC -> (Norm+AOD) -> (Norm+Center) -> (Norm+PWV)');

    % Two-iter counterpart of LAST_Joint_1Iter_AtmosFirst_Split3. Same
    % stage sequence (Norm+Center -> Norm+AOD -> Norm+PWV -> JOINT_FC),
    % but wrapped in the LAST_Joint_2Iter iteration policy: iter 1
    % unweighted followed by a 3-sigma signed-residual clip, then iter 2
    % refit on the cleaned survivor pool with 'combined' MagErr
    % weighting. Reuses LAST_Joint_2Iter_Overrides verbatim so weighting/
    % clip policy stays in one place.
    StageCatalog.LAST_Joint_2Iter_AtmosFirst_Split3 = struct( ...
        'Stages',         LAST_Joint_1Iter_AtmosFirst_Split3_Stages, ...
        'NumRepeats',     2, ...
        'IterOverrides',  LAST_Joint_2Iter_Overrides, ...
        'Description',    'Joint 2-iter atm-first, split atm: (Norm+Center) -> (Norm+AOD) -> (Norm+PWV) -> JOINT_FC (keeps Norm). Iter 1 unweighted + 3sigma clip; iter 2 combined-weighted.');
    ...
    % Gray-transmission, NO Tran2D: fit only Norm analytically. Pair
    % with FunCatalog.GrayFunList AND pass 'UseTran2D', false in
    % CalibArgs so the model has no polynomial component at all. The
    % result is a single scalar Norm that best-fits observed flux to
    % the reference spectrum integrated against a flat transmission -
    % i.e. the "no model" baseline that shows how much of the residual
    % is intrinsic scatter versus explained-by-spectral-modelling.
    StageCatalog.GrayNoTran2D = struct();
    StageCatalog.GrayNoTran2D.StageName      = 'GrayNormOnly';
    StageCatalog.GrayNoTran2D.Method         = 'linear';
    StageCatalog.GrayNoTran2D.FreeParams     = struct( ...
        'Function',  {'Normalization'}, ...
        'Parameter', {'Norm'});
    StageCatalog.GrayNoTran2D.SigmaClip      = false;
    StageCatalog.GrayNoTran2D.SigmaThresh    = 3.0;
    StageCatalog.GrayNoTran2D.SigmaIter      = 0;
    StageCatalog.GrayNoTran2D.MinCalibrators = 30;
    StageCatalog.GrayNoTran2D.Description    = ...
        'Analytical Norm fit on GRAY transmission, no Tran2D (pass UseTran2D=false)';

    % Triangle-transmission JOINT_FC. Same routing as GrayJointFC
    % (Recipe-wrapped so fitMultiStage's JOINT_FC handler is entered),
    % but the model has the synthetic Peak-5700/Edges-3000/11000
    % triangle T(lambda) as the wavelength-dependent term.
    TriangleJointFC_Stage = struct( ...
        'StageName',      'TriangleNormAndTran2D_Linear', ...
        'Method',         'linear', ...
        'FreeParams',     'JOINT_FC', ...
        'SigmaClip',      false, ...
        'SigmaThresh',    3.0, ...
        ...
        'MinCalibrators', 30, ...
        'Tran2DSplit',    'none', ...
        'Description',    'Triangle-transmission JOINT_FC (Norm held at 1, Tran2D LS)');
    StageCatalog.TriangleJointFC = struct( ...
        'Stages',         TriangleJointFC_Stage, ...
        'NumRepeats',     1, ...
        'IterOverrides',  [], ...
        'Description',    ['JOINT_FC linear LS over Tran2D coefficients ', ...
                           'on TRIANGLE (synthetic peaked) transmission']);

    % Triangle transmission, NO Tran2D: fit only Norm analytically.
    % Analogue of GrayNoTran2D but with the synthetic triangle T(lambda)
    % as the wavelength-dependent term. Pair with TriangleFunList AND
    % pass 'UseTran2D', false in CalibArgs.
    StageCatalog.TriangleNoTran2D = struct();
    StageCatalog.TriangleNoTran2D.StageName      = 'TriangleNormOnly';
    StageCatalog.TriangleNoTran2D.Method         = 'linear';
    StageCatalog.TriangleNoTran2D.FreeParams     = struct( ...
        'Function',  {'Normalization'}, ...
        'Parameter', {'Norm'});
    StageCatalog.TriangleNoTran2D.SigmaClip      = false;
    StageCatalog.TriangleNoTran2D.SigmaThresh    = 3.0;
    StageCatalog.TriangleNoTran2D.SigmaIter      = 0;
    StageCatalog.TriangleNoTran2D.MinCalibrators = 30;
    StageCatalog.TriangleNoTran2D.Description    = ...
        'Analytical Norm fit on TRIANGLE transmission, no Tran2D (pass UseTran2D=false)';
