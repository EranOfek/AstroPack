function [Cost, Residuals, DiffMag, PredictedFlux] = calculateCostFunction(CalibData, ParamValues, TransFun, Args)
    % Calculate cost function for transmission-based photometric calibration
    % Input  : - CalibData - Structure with calibrator data with fields:
    %                   .Spec - Cell array {N x 2} with Gaia XP spectra
    %                            (Column 1: Flux values, Column 2: Flux errors)
    ...
    % Example: ParamVals = TransFun.valuesAllPar();
    %          [Cost, Residuals, DiffMag] = imUtil.calib.calculateCostFunction(...
    %              CalibData, ParamVals, TransFun, 'Verbose', true);
    ...
    if any(Args.FieldParams ~= 0)
        ...
        FieldCorrectionMag = telescope.optics.fieldCorrectionLAST(Coords, Args.FieldParams);
