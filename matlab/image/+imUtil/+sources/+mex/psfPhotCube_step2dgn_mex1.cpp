    ...
    for (mwIndex Iim = 0; Iim < Nim; ++Iim) {
        ...
        for (mwIndex Ix = 0; Ix < Nx; ++Ix) {
            ...
            for (mwIndex Iy = 0; Iy < Ny; ++Iy) {
                ...
                const mwIndex Ip = Iy + Ix * Ny;  // column-major
                ...
                if (!(std::isfinite(SXI) && std::isfinite(SYI) && SXI != 0.0 && SYI != 0.0)) {
                    continue;
                }
                const double dPdx = (ToDouble(PSF_XpI[Ip]) - ToDouble(PSF_XmI[Ip])) / (2.0 * SXI);
                const double dPdy = (ToDouble(PSF_YpI[Ip]) - ToDouble(PSF_YmI[Ip])) / (2.0 * SYI);
                ...
                if (!(std::isfinite(W) && std::isfinite(Resid) && std::isfinite(Jx) && std::isfinite(Jy))) {
                    continue;
                }
        ...
        // MATLAB final non-finite cleanup
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (ClassID == mxDOUBLE_CLASS) {
        ComputeSteps<double>(
            ...
            Ny, Nx, Nim, ThisStdMode, UseFitRadius, FitRadius2, MaxStep,
