% (MEX) Given a cube in which each stamp ccontains centered source, perform aperture phot and back estimation around stamp center.
%   Description: Fast aperture photometry and scalar background estimation for
%              centered image stamps (MxMxN cube). Aperture sums are computed
...
%   Syntax: [AperFlux, Back, BackStd, NpixAper, NpixBack] = aperPhotBack_mex(Cube, AperRadii, AnnulusRadii)
%
...
%   [AperFlux, Back, BackStd, NpixAper, NpixBack] = aperPhotBack_mex(Cube, AperRadii, AnnulusRadii);
%   % If you need background-subtracted flux later:
%   k = 3;  % e.g., radius = 4 px
%   Flux_Bsub = AperFlux(:,k) - Back .* NpixAper(k);
%
% Compilation: mex -O CXXFLAGS="$CXXFLAGS -std=c++17 -fopenmp -march=native" LDFLAGS="$LDFLAGS -fopenmp" aperPhotBack_mex.cpp
% See also: regionprops, medfilt2, prctile, accumarray
%           imUtil.sources.mex.aperPhotBackXY_mex