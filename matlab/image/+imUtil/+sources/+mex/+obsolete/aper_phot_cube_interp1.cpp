...
static void aper_phot_cube_hybrid(
...
#endif
    {
        ...
        std::vector<double> binShiftedMask((size_t)Na);
        std::vector<double> Wx_valid((size_t)K);
        std::vector<double> Wy_valid((size_t)M);
        ...
        for (int n = 0; n < N; ++n) {
            ...
            // For background subtraction we need sum(ShiftedMask) over the stamp,
            // not count(Circle). By linearity and separability of the Lanczos shift,
            // each input mask pixel (xi,yi) contributes Wx_valid(xi)*Wy_valid(yi)
            // to the summed shifted mask, where Wx_valid(xi) sums those kernel taps
            // whose target output column lies inside [0,K-1] (and similarly for Y).
            int sxInt, syInt;
            const double fx = frac_from_neg_shift(dx, sxInt);
            const double fy = frac_from_neg_shift(dy, syInt);
            double wx[TAPS], wy[TAPS];
            ...
            std::fill(Wx_valid.begin(), Wx_valid.end(), 0.0);
            std::fill(Wy_valid.begin(), Wy_valid.end(), 0.0);
            for (int xi = 0; xi < K; ++xi) {
                for (int i = 0; i < TAPS; ++i) {
                    const int xo = xi - sxInt - (OFF0 + i);
                    if ((unsigned)xo < (unsigned)K) Wx_valid[(size_t)xi] += wx[i];
                }
            }
            for (int yi = 0; yi < M; ++yi) {
                for (int i = 0; i < TAPS; ++i) {
                    const int yo = yi - syInt - (OFF0 + i);
                    if ((unsigned)yo < (unsigned)M) Wy_valid[(size_t)yi] += wy[i];
                }
            }

            std::fill(binFlux.begin(), binFlux.end(), 0.0);
            std::fill(binArea.begin(), binArea.end(), 0.0);
            std::fill(binShiftedMask.begin(), binShiftedMask.end(), 0.0);
            ...
            for (const auto& p : outpix) {
                ...
                binShiftedMask[(size_t)p.bin] += Wx_valid[(size_t)p.x] * Wy_valid[(size_t)p.y];
            }

            // prefix sums and background subtraction
            double runFlux = 0.0;
            double runArea = 0.0;
            double runShiftedMask = 0.0;
            const double b = Back[n];

            for (int a = 0; a < Na; ++a) {
                runFlux += binFlux[(size_t)a];
                runArea += binArea[(size_t)a];
                runShiftedMask += binShiftedMask[(size_t)a];
                outFlux[n + (mwSize)N * (mwSize)a] = runFlux - b * runShiftedMask;
