...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	if (mxIsClass(prhs[0], "uint16")){
		if (isnan(pad_value) || pad_value<0) {
			pad_value=0; // cannot have NaN or negative values in the uint16 matrix
		}
		
		if (isnan(replace_value) || replace_value<0) { 
			replace_value=0; // cannot have NaN or negative values in the uint16 matrix
		}
		...
		if (isnan(pad_value) || pad_value<0) {
			pad_value=0; // cannot have NaN or negative values in the boolean matrix
		}
		
		if (isnan(replace_value) || replace_value<0) {
			replace_value=0; // cannot have NaN or negative values in the boolean matrix
		}
		...
		for (int c=0; c<num_cuts; c++) {
			for (int p=0; p<pages; p++) {
				for (int j=0; j<cut_size; j++) {
					for (int i=0; i<cut_size; i++) { // use the one-by-one method for non-zero pad values
	...
	for (int c=0; c < num_cuts;  c++){
		...
		for (int p=0; p<pages; p++) {
			...
			if (use_memcpy) {
				memcpy(dst, src, N); // use the new version of the code (default) based on memcpy
			}
			else {
				for (int i=push_y; i<cut_size_y; i++) {
					memcpy(&dst[i*num_bytes], &src[i*num_bytes], num_bytes); // old code (for benchmarking) goes over data one-by-one (copy num_bytes in each location)
				}
			}
		}
		...
		for (int c=0; c<num_cuts; c++) {
			...
			for (int p=0; p<pages; p++) {
				...
				if (replace_value_bytes==0) 
					memset(src, 0, N); // for zero use quick replace (memset)
				else {
					for (int i=push_y; i<cut_size_y; i++) {
						memcpy(&src[i*num_bytes], &replace_value_bytes, num_bytes); // for non-zero must go over memory locations one-by-one (copy num_bytes in each location)
					}
				}
			}
		}
