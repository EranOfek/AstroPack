function CCDSEC = remove_edge_section(SizeOriginalImageIJ, OverScan, Dir)
    ...
    %            If empty, then will attempt to estimate overscan dir, by
    %            chooseing the direction that covers a full axis.
    %            Default is [].
    ...
    if isempty(Dir)
        % attempt to estimate overscan dir, by chooseing the direction that
        % covers a full axis

        Nx = OverScan(2) - OverScan(1) + 1;
        Ny = OverScan(4) - OverScan(3) + 1;

        if Nx==SizeOriginalImageIJ(2)
            Dir = 1;
        else
            Dir = 2;
        end
