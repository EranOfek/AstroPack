...
// function prototypes
void shift_add(float *line_out, float *line_prev1, float *line_prev2, int N, int gap); 
void add_lines(float *M_out, float *M_prev, int *output_size, int *input_size, double *dy, int ij_start, int ij_end, int debug_level=0);
...
void mexFunction( int nlhs, mxArray *plhs[],
	...
	float *M_out=(float*) mxGetData(prhs[0]);
	...
	float *M_prev=(float*) mxGetData(prhs[1]);
...
void add_lines(float *M_out, float *M_prev, int *output_size, int *input_size, double *dy, int ij_start, int ij_end, int debug_level){
	...
	for(int ij=ij_start;ij<ij_end;ij++){
		...
		float *line_out=&M_out[i*output_size[0]+j*output_size[0]*output_size[1]];
		float *line_prev1=&M_prev[i_prev*input_size[0]+j_prev*input_size[0]*input_size[1]];
		float *line_prev2=&M_prev[(i_prev+1)*input_size[0]+j_prev*input_size[0]*input_size[1]];
