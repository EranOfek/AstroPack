function [Result] = forcedPhotNew(Obj, Args)
    ...
    arguments
        ...
        Args.ZP                      = 25;
        % Flux->magnitude conversion for the MAG_* columns:
        ...
        if IsGood(Iobj)
            ...
            for Icol=1:1:Ncol
                ...
                switch Args.ColCell{Icol}
                    ...
                    case 'MAGERR_PSF'
                        MagErrPsf = 1.086./ResultPSF.SNm;
                        if strcmp(Args.MagType, 'mag')
                            % MAG_PSF is NaN for non-positive flux (and SNm is
                            % signed, so this would otherwise go negative).
                            MagErrPsf(~(ResultPSF.Flux>0)) = NaN;
                        end
                        Data(:,K) = MagErrPsf;
                    case 'CHI2DOF'
                    ...
                    case 'FLUXERR_APER_1'
                        if isempty(FluxErrRelAper)
                            FluxErrRelAper = i_fluxErrRelAper(Aper);
                        end
                    ...
                    case 'MAG_APER_2'
                        if strcmp(Args.MagType, 'mag')
                            Data(:,K) = convert.magnitude(Aper.AperPhot(:,2), 10.^(0.4.*Args.ZP));
                        else
                            Data(:,K) = convert.luptitude(Aper.AperPhot(:,2), 10.^(0.4.*Args.ZP));
                        end
                    case 'MAG_APER_3'
                        if strcmp(Args.MagType, 'mag')
                            Data(:,K) = convert.magnitude(Aper.AperPhot(:,3), 10.^(0.4.*Args.ZP));
                        else
                    ...
                    case 'MAGERR_APER_1'
                        ...
                        if strcmp(Args.MagType, 'mag')
                            ...
                            MagErrAper(~(Aper.AperPhot(:,1)>0)) = NaN;
                        end
                        Data(:,K) = MagErrAper;
                    ...
                    case 'MAGERR_APER_2'
                        ...
                        if strcmp(Args.MagType, 'mag')
                            ...
                            % error is divided by it, so it must follow.
                            MagErrAper(~(Aper.AperPhot(:,2)>0)) = NaN;
                        end
                        Data(:,K) = MagErrAper;
                    case 'MAGERR_APER_3'
                        if isempty(FluxErrRelAper)
                        ...
                        if strcmp(Args.MagType, 'mag')
                            ...
                            % error is divided by it, so it must follow.
                            MagErrAper(~(Aper.AperPhot(:,3)>0)) = NaN;
                        end
                        Data(:,K) = MagErrAper;    
                    case 'APER_AREA_1'
                    ...
                    case 'BACKMAG_ANNULUS'
                        if strcmp(Args.MagType, 'mag')
                            Data(:,K) = convert.magnitude(Aper.AnnulusBack, 10.^(0.4.*Args.ZP));
                        else
...
%%%
function FluxErrRel = i_fluxErrRelAper(Aper)
    % Relative aperture flux error (dF/F), shared by the FLUXERR_APER_* and
    % MAGERR_APER_* columns. Non-positive flux: the relative error is
    % meaningless (and would be negative after the division) - NaN, matching
    % the NaN MAG (issue #1135).
    FluxErrRel = sqrt(Aper.AperPhotErr.^2 + (Aper.AnnulusStd./sqrt(Aper.AnnulusBackArea)).^2)./Aper.AperPhot;
    FluxErrRel(~(Aper.AperPhot>0)) = NaN;
end
