function [ResultFit, AI, CatName] = astrometrySingleImage(AI, Args)
    ...
    %   an InitWCS which will be used with astrometryRefine.
    %   If astrometryRefine is failed, then execute astrometryCore.
    %   If astrometryCore is failed, then attempt again with somewhat
    %   different parameters.
    %
    % Input  : - A scalar AstroImage object.
    %          * ...,key,val,... 
    ...
    %            'RefRangeMag' - Reference catalog magnitude range, shifted
    %                   by the exposure time correction (see
    %                   'RefRangeMagExpTimeFun') and passed to both
    %                   astrometryRefine and astrometryCore. In crowded
    %                   fields a too deep range is counter productive: the
    %                   neighbour rejection applied by getAstrometricCatalog
    %                   then removes almost all the reference stars.
    %                   Default is [10 17.0].
   ...
   if ~isempty(Args.RA) && ~isempty(Args.Dec)
    ...
    if Args.SkipSolved && ~isempty(AI.WCS) && AI.WCS.Success
        ...
        Nsrc = size(AI.CatData.Catalog,1);
        if ~isempty(InitWCS) && InitWCS.Success
            ...
            if ~isempty(ResultFit.WCS)
                Success = ResultFit.WCS.Success;
            end
            
        end % if ~isempty(InitWCS) && InitWCS.Success
    
        if ~Success
            % failed or did not even attempted
    ...
    end %if Args.SkipSolved && ~isempty(AI.WCS) && AI.WCS.Success
