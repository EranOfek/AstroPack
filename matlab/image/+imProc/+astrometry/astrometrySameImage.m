function [ResultFit, AI, CatName] = astrometrySameImage(Obj, Args)
    ...
    for Iai=1:1:Nai
        ...
        if ~isempty(InitWCS) && InitWCS.Success
            ...
            [ResultFit(Iai), AI(Iai), CatName] = imProc.astrometry.astrometryRefine(AI(Iai),...
                                                                               'WCS',InitWCS, ...
            ...
            if ~isempty(ResultFit(Iai).WCS)
                Success(Iai) = ResultFit(Iai).WCS.Success;
                InitWCS      = ResultFit(Iai).WCS;  % set the InitWCS to that of the previously solved image
            end
        ...
        if ~Success(Iai)
            ...
            [ResultFit(Iai), AI(Iai), CatName] = imProc.astrometry.astrometryCore(AI(Iai),...
                                                                                 'Tran',Args.Tran,...
                                                                                 'RA',RA,...
                                                                                 'Dec',Dec,...
                                                                                 'CooUnits',Args.CooUnits,...
