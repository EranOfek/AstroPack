function [Result, ResFit] = photometricZP_AB(Obj, Args)
    ...
    %                   If empty, loads from YAML configuration.
    %                   Default is [].
    ...
    %                   Default is [12 16].
    %            'FluxColName' - LAST flux column name.
    %                   Default is 'FLUX_APER_3'.
    %            'UpdateHeader' - Update AstroImage header with ZP.
    %                   Default is true.
    %            'UpdateMagCols' - Update magnitude columns with ZP.
    %                   Default is true.
    %            'MagColName2update' - Magnitude columns to update.
    %                   Default is 'MAG_'.
    %            'SignZP' - Sign of ZP to add (+1 or -1).
    %                   Default is 1.
    %            'MagSys' - Magnitude system ('AB' or 'Vega').
    %                   Default is 'AB'.
    %            'MagZP' - Instrumental zero point.
    %                   Default is 25.
    %            'CreateNewObj' - Copy input object.
    %                   Default is false.
    ...
    %          ResFit = imProc.calib.photometricZP_AB(AI);
    %          % Advanced usage with custom YAML config
    %          [Result, ResFit] = imProc.calib.photometricZP_AB(AI, ...
    %                                'YAMLConfigPath', '/path/to/custom_config.yml', ...
    %                                'SearchRadius', 1.5, 'Verbose', true);
...
tic
    ...
    AbsData = astro.transmission.loadAbsorptionInterpolantsSMARTS('Verbose', Args.Verbose);
    ...
    % Copy object if requested, otherwise work on original
    if Args.CreateNewObj
        Result = Obj.copy();
    else
        Result = Obj;  % Work directly on input object (no copy created)
    end
    ...
    for Iobj = 1:Nobj
        ...
        [SequenceResults, FinalCalibData, FinalFieldParams] = imUtil.calib.runOptimizationSequence_YAML(...
...
function Obj = updateMagnitudeColumns(Obj, ZP, MagColPattern, SignZP, FieldParams, Verbose)
    ...
    if any(FieldParams ~= 0)
        ...
        FieldCorrectionMag = telescope.optics.fieldCorrectionLAST(Coords, FieldParams);
...
function CalibTable = createCalibratorTable(CalibData, TransFun, FieldParams, DiffMagRes, FluxColName, Verbose)
    ...
    [~, ~, ~, PredictedFlux] = imUtil.calib.calculateCostFunction(...
    ...
    if any(FieldParams ~= 0)
        Coords = [X(:), Y(:)];
        FieldCorrection = telescope.optics.fieldCorrectionLAST(Coords, FieldParams);
    else
    ...
    if Verbose
        ...
        fprintf('  Columns: RA, Dec, X, Y, Observed/Predicted Flux, DiffMagRes, Field Correction\n');
