function [Result, ResFit] = photometricZP_AB_AstroCatalog_TEST(Obj, Args)
    % Transmission-based absolute photometric calibration using Gaia XP spectra
    % TEST VERSION: Supports both AstroImage and AstroCatalog input
    ...
    %            'Airmass' - Airmass value (REQUIRED for AstroCatalog input). Default is [].
    %            'Temperature' - Temperature in Celsius (REQUIRED for AstroCatalog input). Default is [].
    ...
    % Example: % Basic usage with AstroImage (metadata from header)
    %          AI = io.files.load2('LAST_image.mat');
    %          [Result, ResFit] = imProc.calib.photometricZP_AB_AstroCatalog_TEST(AI, 'Verbose', true);
    %
    %          % Usage with AstroCatalog (must provide metadata)
    %          AC = AstroCatalog(...);
    %          [Result, ResFit] = imProc.calib.photometricZP_AB_AstroCatalog_TEST(AC, ...
    %                                'Airmass', 1.2, ...
    %                                'Temperature', 15.0, ...
    %                                'Verbose', true);
    ...
    arguments
        ...
        % Metadata (required for AstroCatalog input)
        Args.Airmass = []  % Airmass value (required for AstroCatalog)
        Args.Temperature = []  % Temperature in Celsius (required for AstroCatalog)
  ...
  IsAstroImage = false;
    ...
    for Iobj = 1:Nobj
        ...
        if IsAstroImage
            ...
            % For AstroCatalog, use provided metadata
            if isempty(Args.Airmass) || isempty(Args.Temperature)
                error('AstroCatalog input requires Airmass and Temperature arguments.\n%s', ...
                      'Usage: photometricZP_AB_AstroCatalog_TEST(Cat, ''Airmass'', 1.2, ''Temperature'', 15.0)');
            end

            Metadata = struct();
            Metadata.Airmass = Args.Airmass;
            Metadata.Temperature = Args.Temperature;

            if Args.Verbose
                fprintf('Using provided metadata for AstroCatalog input:\n');
                fprintf('  Airmass: %.3f\n', Metadata.Airmass);
        ...
        % TransFun already contains all optimized parameters (Norm, atmospheric, etc.)
        ...
        if isfield(CalibData.Metadata, 'Aperture_area_m2') && ~isnan(CalibData.Metadata.Aperture_area_m2)
            Ageom = CalibData.Metadata.Aperture_area_m2;
        else
...
function CalibTable = createCalibratorTable(CalibData, TransFun, FieldParams, Residuals, FluxColName, Verbose)
    ...
    %          - Residuals - Final residuals from optimization [N x 1]
    ...
    [~, ~, DiffMag, PredictedFlux] = imUtil.calib.calculateCostFunction_DRAFT(...
    ...
    if Verbose
        ...
        fprintf('  Columns: RA, Dec, X, Y, Observed/Predicted Flux, Magnitudes, Residuals, Field Correction\n');
