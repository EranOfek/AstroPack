...
classdef AstroCatalog < handle %ImageComponent
    ...
    methods % Constructor
        ...
        function Obj = AstroCatalog(n) % varargin)
            persistent i;
            if isempty(i)
                i = 0;                
                fprintf('Ctor: init i\n');
       ...
       function Obj = make(Obj, n) % varargin)
            persistent j;
            if isempty(j)
                j = 0;                
                fprintf('make: init j\n');
       ...
       function fill(Obj, val)
        ...
        function Obj = read(Obj, AnotherObj, Args)
            ...
            %          AC = AstroCatalog(A);
            %          AC = AstroCatalog(A,'ColNames',{'RA','Dec'},'ColUnits',{'rad','rad'});
            ...
            if isempty(AnotherObj)
                ...
                if ischar(AnotherObj) || iscell(AnotherObj) || isstring(AnotherObj)
                    ...
                    for Ilist=1:1:Nlist
                        disp(Ilist);
                        % create a single empty AstroCatalog (recursion)
                        if Ilist > numel(Obj)
                           fprintf('Adding: %d\n', Ilist);
                ...
                elseif isa(AnotherObj,'AstCat') || isa(AnotherObj,'catCl')
                    ...
                    for Iobj=1:1:Nobj
                        ...
                        if isempty(Args.ColNames)
                            Obj(Iobj).ColNames  = AnotherObj(Iobj).ColCell;
                        else
...
%     end
    ...
    methods (Static)  % static methods
       function VarNames = default_ColNames(Ncol)
            % create a default ColNames with N columns
            % Package: @AstroCatalog (Static)
            % Input  : - Number of columns
            % Example: AstroCatalog.default_ColNames(5)
       ...
       function Ans = compare_colnames(ColNames1, ColNames2)
            ...
            %            are identical.
            % Example: AstroCatalog.compare_colnames({'a','b'},{'a','b'})
    ...
    methods % get general info
        ...
        function Result = isColIdentical(Obj, ColNames)
            % Check if ColNames in an AstroCatalog object is identical to another ColNames
            % Package: @AstroCatalog
            ...
            Result = false(size(Obj));
            for Iobj=1:1:Nobj
                Result(Iobj) = all(AstroCatalog.compare_colnames(Obj(Iobj).ColNames, ColNames));
            end
        end
    ...
    methods  % columns get/edit
        ...
        function Obj = insertCol(Obj, Data, Pos, NewColNames)
            ...
            % Example: A=AstroCatalog; A.Catalog=rand(10,3); A.ColNames={'a','b','c'}; insertCol(A,ones(10,2),'c')     
            ...
            if isa(Data,'AstroCatalog')
                ...
                for Iobj=1:1:Nobj
                    ...
                    Obj(Iobj).ColNames = AstroCatalog.insert_column(Obj(Iobj).ColNames, AstroCatalog.default_colnames(size(Data,2)), ColInd);
                    if ~isempty(NewColNames)
                        Obj(Iobj).ColNames(ColInd) = NewColNames;
                    end
                end
    ...
    methods % applay functions and overloads
        ...
        function Result = fun_unary(Obj, Operator, OperatorArgs, Columns, UpdateObj)
            ...
            % Author : Eran Ofek (Mar 2021)
            % Example: AC=AstroCatalog; AC.Catalog = rand(100,3); AC.ColNames={'a','n','s'}; AC.fun_unary(@sin)
