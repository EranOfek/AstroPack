...
%         gridAxes = {'RA','Dec','Color'}
    ...
    % static methods for populating the wcsCl object
    % declaration of the static methods in external files
    methods (Static)
        
        [Phi,Theta] = alphadelta2phitheta(Alpha,Delta,PhiP,AlphaP,DeltaP,Units)
        
        ProjClass = classify_projection(ProjType)
        
        [X,Y]     = grid_coo2xy(gridCoo,gridLon,gridLat,varargin)
        
        [Lon,Lat] = grid_xy2coo(gridCoo,gridLon,gridLat,varargin)
        
        [Alpha,Delta]=phitheta2alphadelta(Phi,Theta,PhiP,AlphaP,DeltaP,Units)
        
        W = pop_example
        
        [X,Y] = projection_TAN(Long,Lat,R,CenterVec)
        
        [X,Y] = projectTAN_coo2xy(Lon,Lat,CRVAL,Units)
        
        [Lon,Lat] = projectTAN_xy2coo(X,Y,CRVAL,Units)
        
        [Flag,W] = iswcsOk(W)
        
        [PolyPV] = polyPVdef;
        
        [Ind,PolyPV] = poly2tpvInd(Poly_Xdeg,Poly_Ydeg,Poly_Rdeg)
