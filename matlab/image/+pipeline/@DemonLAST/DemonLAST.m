...
%             end
    ...
    methods % go over files
        ...
        function [Result,OutTable]=findAllVisits(Obj, Args)
            ...
            for Ind=1:1:Nr
                % 14 col                
                OutTable(Ind,:) = [Result(Ind).Keys(1).MOUNTNUM, Result(Ind).Keys(1).CAMNUM, Result(Ind).JD,...
                                   Result(Ind).Keys(1).RA1, Result(Ind).Keys(1).DEC1, ...
                                   Result(Ind).Keys(1).RA2, Result(Ind).Keys(1).DEC2, ...
                                   Result(Ind).Keys(1).RA3, Result(Ind).Keys(1).DEC3, ...
                                   Result(Ind).Keys(1).RA4, Result(Ind).Keys(1).DEC4, ...
                                   Result(Ind).Keys(1).FWHM, Result(Ind).Keys(1).LIMMAG, Result(Ind).Keys(1).BACKMAG];
