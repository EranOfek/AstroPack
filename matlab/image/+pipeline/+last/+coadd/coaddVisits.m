function [CI, AI] = coaddVisits(In, Args)
    ...
    %          * ...,key,val,... 
    %            'TableOrigin' - Options are:
    %                   'last_visits' - table in DB.
    %                   'VisitCat' - catalog generated by pipeline.DemonLAST/findAllVisits
    %                   Default is 'last_visits'.
    ...
    %
    %          CI=pipeline.last.coadd.coaddVisits(T(F,:),'CropID',10);
    %       
    ...
    if istable(In)
        switch lower(Args.TableOrigin)
            case 'last_visits'
                [AFN, AllPaths,AllFiles] = pipeline.last.queryDB.table2path(In, Args.table2pathArgs{:});
                List = AllPaths; %join([AllPaths, AllFiles], filesep, 2);
            case 'visitcat'
            ...
            otherwise
                error('Unknown TableOrigin option');
        end
    else
        List = In;
    end
   ...
   % Args.FindSrc=false;
    if Args.FindSrc
        ...
        CI = imProc.sources.psfFitPhot(CI);
