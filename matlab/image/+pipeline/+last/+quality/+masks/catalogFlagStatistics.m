function Report = catalogFlagStatistics(DataPath, Args)
    ...
    %   is hardcoded. This mirrors pipeline.last.quality.masks.maskBitStatistics
 ...
 %            AC = AstroCatalog(FileName);
        ...
        for Ib = 1:Nbits
            if NumSources > 0
                ...
                NumBitSrc  = 0;
 Rmask = pipeline.last.quality.masks.maskBitStatistics(DataPath, ...
'BitNames', {'CoaddLessImages'}, ...
'AccumulateMaps', true);
pipeline.last.quality.masks.plotMaskBitStatistics(Rmask);                FracBitSrc = 0;
            end
