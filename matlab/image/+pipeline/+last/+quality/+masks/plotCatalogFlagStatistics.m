function Figs = plotCatalogFlagStatistics(Report, Args)
    ...
    %            pipeline.last.quality.masks.catalogFlagStatistics with
    ...
    % Example: R = pipeline.last.quality.masks.catalogFlagStatistics(DataPath, ...
    %                  'BitNames', {'CoaddLessImages'});
    %          % Density of flagged sources (counts per bin):
    %          pipeline.last.quality.masks.plotCatalogFlagStatistics(R);
    %          % Scatter of flagged sources:
    %          pipeline.last.quality.masks.plotCatalogFlagStatistics(R, 'Mode','scatter');
