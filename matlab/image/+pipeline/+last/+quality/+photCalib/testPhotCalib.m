function Result = testPhotCalib(Args)
    ...
    %     pipeline.last.quality.photCalib.testPhotStability.
    ...
    %     pipeline.last.quality.photCalib.testPhotFitQuality.
    %
    %              Delegated functions:
    ...
    %          R = pipeline.last.quality.photCalib.testPhotCalib('Modes', {'percrop','perimage'}, ...
    %              'CropsToAnalyze', [10 19]);
    %
    %          % Visit-level modes:
    %          R = pipeline.last.quality.photCalib.testPhotCalib('Modes', ...
    %              {'percrop','perset'}, 'VisitRefZP', 'crop_median');
    %          R = pipeline.last.quality.photCalib.testPhotCalib('Modes', ...
    %              {'percrop','shapeset'});
    %
    %          % Load coadd files from .mat visit list:
    %          R = pipeline.last.quality.photCalib.testPhotCalib('DataDir', '', ...
    ...
    %          R = pipeline.last.quality.photCalib.testPhotCalib('VisitDirs', ...
    %              ["/path/to/visit1", "/path/to/visit2"], 'FileType', 'coadd');
    %
    %          % Skip zp_meddiff on original mags (requires ForceRecalc):
    %          R = pipeline.last.quality.photCalib.testPhotCalib('ApplyRelZP', false, ...
    %              'ForceRecalc', true);
    %
    ...
    %          pipeline.last.quality.photCalib.plotPhotScatter(R.MS, 'Modes', {'percrop'}, 'MinEpochs', 15);
    %          pipeline.last.quality.photCalib.plotPhotStdDiff(R.MS, 'Modes', {'percrop','perimage'});
    %          pipeline.last.quality.photCalib.plotPhotMosaic(R, 'Modes', {'percrop'});
    %          pipeline.last.quality.photCalib.plotPhotTransmission(R, 'Modes', {'percrop'});
    %          pipeline.last.quality.photCalib.plotPhotIntegralT(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotFWHM(R);  % uses R.HeaderData
    %          pipeline.last.quality.photCalib.plotPhotFittedParams(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotFittedParams(R.PC, ...
    %              'ParamNames', {'TauAod500','PWV_cm','Norm','QE_Center_Ang'});
    %          pipeline.last.quality.photCalib.plotPhotParamCorr(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotParamCorr(R.PC, 'ColorByCrop', true);
    %          pipeline.last.quality.photCalib.plotPhotParamSynchrony(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotShapeSynchrony(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotAsympRMS(R);
    %
    %          % Fit quality only (from cached PC):
    %          pipeline.last.quality.photCalib.testPhotFitQuality(R.PC);
    %          pipeline.last.quality.photCalib.testPhotFitQuality(R.PC, ...
    %              'ExtraXFields', {'FLAGS','PSF_CHI2DOF'});
    %
    %          % Fit quality from single AstroImage (calibrates internally):
    %          pipeline.last.quality.photCalib.testPhotFitQuality(AI);
    %
    %          % ConstBand mode:
    %          CBP = [R.PC.percrop{:}].buildConstBandParams('Verbose', true);
    %          R = pipeline.last.quality.photCalib.testPhotCalib(..., ...
    %              'ApplyConstBand', true, 'ConstBandParams', CBP, 'ForceRecalc', true);
    %
    %          % Save all figures:
    %          R = pipeline.last.quality.photCalib.testPhotCalib(..., 'SaveFig', true);
    %
    ...
    %{         
                 ...
                 pipeline.last.quality.photCalib.plotPhotScatter(S.MS_all, 'Modes', Modes, 'MinEpochs', 15);
                 pipeline.last.quality.photCalib.plotPhotStdDiff(S.MS_all, 'Modes', Modes, 'MinEpochs', 15);
    %}
    %
    %{
                 R = pipeline.last.quality.photCalib.testPhotCalib('DataDir', '', ...
    ...
    if Args.ApplyConstBand
        ...
        elseif iscell(Args.ConstBandParams)
            % Cell array of PC_all — flatten, then build CBP
    ...
    pipeline.last.quality.photCalib.testPhotFitQuality(Calib.PC, ...
        'CropsToAnalyze', Args.CropsToAnalyze, ...
        'OverlayTrend', Args.OverlayTrend, ...
        'TrendBinWidth', Args.TrendBinWidth, ...
        'SaveFig', false, ...
