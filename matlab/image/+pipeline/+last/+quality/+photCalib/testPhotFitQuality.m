function Result = testPhotFitQuality(Input, Args)
    ...
    %          pipeline.last.quality.photCalib.testPhotFitQuality(R);
    %
    ...
    %          pipeline.last.quality.photCalib.testPhotFitQuality(AI_cell);
    %
    ...
    %          pipeline.last.quality.photCalib.testPhotFitQuality(R.PC, 'PlotColor', false);
    %
    %          % Save figures:
    %          pipeline.last.quality.photCalib.testPhotFitQuality(R.PC, ...
    ...
    %          pipeline.last.quality.photCalib.plotPhotResiduals(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotResidualsRMS(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotResidualsColor(R.PC, 'PlotMagColor', true);
    %          pipeline.last.quality.photCalib.plotPhotResidualsBg(R.PC);
    %          pipeline.last.quality.photCalib.plotPhotResidualsVs(R.PC, 'XField', 'FLAGS');
    %          pipeline.last.quality.photCalib.plotPhotResidualsAirmass(R.PC);
    ...
    pipeline.last.quality.photCalib.plotPhotResiduals(PC, ...
    ...
    pipeline.last.quality.photCalib.plotPhotResidualsRMS(PC, ...
        'CropsToAnalyze', Args.CropsToAnalyze);

    % 3. Residuals vs GAIA color
    if Args.PlotColor
        pipeline.last.quality.photCalib.plotPhotResidualsColor(PC, ...
            'CropsToAnalyze', Args.CropsToAnalyze, ...
            'OverlayTrend', Args.OverlayTrend, ...
    ...
    pipeline.last.quality.photCalib.plotPhotResidualsBg(PC, ...
        'CropsToAnalyze', Args.CropsToAnalyze, ...
        'OverlayTrend', Args.OverlayTrend);

    % 5. Residuals vs X-XPEAK, Y-YPEAK
    pipeline.last.quality.photCalib.plotPhotResidualsVs(PC, ...
        'XField', 'X-XPEAK', ...
        'CropsToAnalyze', Args.CropsToAnalyze, ...
        'OverlayTrend', Args.OverlayTrend);
    pipeline.last.quality.photCalib.plotPhotResidualsVs(PC, ...
    ...
    pipeline.last.quality.photCalib.plotPhotResidualsAirmass(PC, ...
        'CropsToAnalyze', Args.CropsToAnalyze, ...
        'OverlayTrend', Args.OverlayTrend);
