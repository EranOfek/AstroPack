function Result = calibratePhotModes(AI, Args)
    ...
    % Example: R = pipeline.last.quality.photCalib.calibratePhotModes(AI);
    %          R = pipeline.last.quality.photCalib.calibratePhotModes(AI, ...
        ...
        if ~UseCache
            ...
            for Iv = 1:Nvisits
                ...
                for Ic = 1:Ncrop_v
                    ...
                    PCmode = PC_all{Iv}(Ic).derivePC(RefParamVec, ...
                        'UseRefNorm', Sett.UseRefNorm, ...
                        'NormTran2DToCenter', Sett.NormTran2D);
                    Cats_all{Iv}(Ic) = PCmode.addMag( ...
                        AIcopy(Ic).CatData, 'MagSystem', 'AB');
                end
            ...
            for Iv = 1:Nvisits
                ...
                for Ic = 1:Ncrop_v
                    ...
                    if IsShapeset
                        ...
                        PCmode = PC_all{Iv}(Ic).derivePC(CropParams, ...
                            'UseRefNorm', false, ...
                        ...
                        PCvis = PC_all{Iv}(Ic).derivePC(CropParams, ...
                            'UseRefNorm', true, ...
                            'NormTran2DToCenter', DoNormTran2D);
                        ZPvisitBase = PCvis.evaluateZP();

                        if isfinite(ZPvisitBase) && Ic <= numel(TargetZP) && isfinite(TargetZP(Ic))
                            DeltaZP = TargetZP(Ic) - ZPvisitBase;
                            AllPar = PCvis.TransModel.getAllFunPar();
                            AllPar.Val(NormIdx) = AllPar.Val(NormIdx) * 10^(DeltaZP / 2.5);
                            PCvis.TransModel.setAllFunPar(AllPar);
                        end

                        Cats_all{Iv}(Ic) = PCvis.addMag( ...
                            AIcopy(Ic).CatData, 'MagSystem', 'AB');
                    end
                end
