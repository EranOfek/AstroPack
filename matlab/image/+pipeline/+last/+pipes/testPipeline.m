function [Result] = testPipeline(Args)
    ...
    %            'AddColor'         - attach the Gaia colour BP_RP to the coadd catalogs
    %                                  (issue #1289); forwarded to pipelineI, 'v1' only (def. true)
    %            'AddColorEpochs'   - also attach it to every epoch catalog (def. false)
    ...
    arguments
        ...
        Args.Nworkers          = 24    % number of matlab parallel processes 
        Args.ConnectDB         = false % connect a DB 
        ...
        Args.Backup            = false % copy the pipeline products to a remote backup server (e.g., Euclid)
        ...
        Args.AddColor logical  = true  % attach the Gaia colour BP_RP to the coadd catalogs (issue #1289); passed on to pipelineI ('v1' only)
        Args.AddColorEpochs logical = false % also attach it to every epoch catalog
    ...
    switch lower(Args.PipelineVersion)
        ...
        case 'v1'            
            ...
            % run the pipeline
            D.main('StopWhenDone',Args.StopWhenDone,'ConnectDB',Args.ConnectDB, ...
                'SaveEpochProduct',{'Image','Mask','Cat','PSF'},'StopButton',false,...
                'StartJD', StartJD, 'EndJD', EndJD, ...
                'RegenCalib', Args.RegenCalib, ...
                'MinInGroup',Args.MinInGroup, ...                
                'UpdateStatusFile', false, ...
                'UpdateRedis', Args.UpdateRedis, ...
                'pipelineIArgs', {'UseParfor',Args.UseParfor,'Nworkers',Args.Nworkers,...
                                  'MatchMethod','mex',...
                                  'prePrepArgs',{'AstroImageReadArgs',{'UseMex', true}},...
                                  'multiIterExtractorArgs',Args.multiIterExtractorArgs,...
                                  'astrometryVisitSubImageArgs',{'astrometryRefineArgs',{'boundingCircleArgs',{'UseMex', true}}},...
                                  'photometricZPArgs',{'boundingCircleArgs',{'UseMex', true}},...
                                  'procCoaddArgs',{'astrometryRefineArgs',{'boundingCircleArgs',{'UseMex', true}},...
                                                      'photometricZPArgs',{'boundingCircleArgs',{'UseMex', true}}},...
                                  ...
                                  'AddColorEpochs', Args.AddColorEpochs,...
                                  'AddColorForced', Args.AddColorForced,...
                                  },...
                'MoveNew2Raw',false,...
                'FocusTreatment', 'keep',...
                'FailMethod','report',...
                'StaticRAWDir',Args.StaticRAWDir,...
                'Backup',Args.Backup,...
        ...
        case 'v0'
            ...
            D.main('StopWhenDone',true,'Insert2DB',false, 'SaveEpochProduct',{'Image','Mask','Cat','PSF'},'StopButton',false,...
                ...
                'CompressedRAW',Args.CompressedRAW,...
                'LocalBase',Args.LocalPath,...
                'MoveRaw2OutputDir',false,...
                'Backup',Args.Backup, 'DebugMode', true,...
                'DbHost','10.150.28.18');
