...
Load PTF fits images and catalogs into a SIM object Package: VO.PTF Description: Load PTF fits images and catalogs into a SIM object. The catalog names are constructed from the file names. Either sextractor or daophot catalogs can be loaded.
...
Get PTF corrected images from the IPAC archive Package: VO.PTF Description:  Given coordinates or PTF field+CCDID, get the link to the images and associated catalogs and mask files. The program can also retrieve the files.
