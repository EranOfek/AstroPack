...
if (nargin>1 && ischar(varargin{1})==0)
   ...
   for I=1:1:N
      ...
      %--- plot Y error bars ---
      if (xor(isnan(Data(I,ColYe(1))),isnan(Data(I,ColYe(2))))==1)
         %--- plot lower/upper limit arrow ---
	 if (isnan(Data(I,ColYe(1)))==1)
	    ArrowStart = Data(I,ColY)+Data(I,ColYe(2));
            ArrowEnd   = Data(I,ColY);
         else
	    ArrowStart = Data(I,ColY)-Data(I,ColYe(1));
            ArrowEnd   = Data(I,ColY);
         end
%[	    ArrowStart, ArrowEnd]
	 [HarrowLine(I), HarrowHead(I)] = arrow2([Data(I,ColX);Data(I,ColX)],[ArrowStart;ArrowEnd],...
                                                  ArrowLength.*abs(ArrowStart-ArrowEnd),...
						  ArrowBase.*abs(ArrowStart-ArrowEnd));
