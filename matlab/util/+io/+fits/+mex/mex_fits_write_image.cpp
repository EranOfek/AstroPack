...
const char* omitted_keys_list[] = {"SIMPLE", "NAXIS", "NAXIS1", "NAXIS2", "BITPIX", "EXTEND", 0};
...
void printValue(char* key, mxArray* valueElement, char* value, size_t valueSize) 
{
    if (mxIsChar(valueElement)) {
        // Add single quotes for string values, doubling any embedded single
        // quote as the FITS standard's quoted-string convention requires
        // (a quote inside a quoted string is written twice to escape it) -
        // previously left unescaped here, producing a malformed card for
        // any string value containing an apostrophe. See issue #1212.
        char tempStr[2048];
