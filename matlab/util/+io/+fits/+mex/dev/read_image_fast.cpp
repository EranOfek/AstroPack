...
mxArray* readHeaderCell(fitsfile* fptr)
{
    ...
    for (int i = 0; i < nkeys; i++) {
        ...
        // Read keyword line (ONE CFITSIO call only)
        fits_read_keyn(fptr, i + 1, key, value_str, comment, &status);
        checkStatus(status);

        // ---- Column 1: keyword ----
        mxSetCell(cell, i, mxCreateString(key));

        // ---- Column 2: parsed value ----
        mxArray* val = nullptr;
        ...
        if (endptr != value_str && *endptr == '\0') {
            // Pure number
            val = mxCreateDoubleScalar(d);
        }
        else if (value_str[0] == 'T' && value_str[1] == '\0') {
            val = mxCreateLogicalScalar(true);
        }
        else if (value_str[0] == 'F' && value_str[1] == '\0') {
            val = mxCreateLogicalScalar(false);
        }
        else {            
            // --- FITS string handling ---
            size_t len = strlen(value_str);
            
            if (len >= 2 && value_str[0] == '\'' && value_str[len - 1] == '\'') {
                // Strip outer quotes and unescape ''
                char* buf = (char*)mxMalloc(len); // worst case same size
                size_t j = 0;
                
                for (size_t i = 1; i < len - 1; i++) {
                    if (value_str[i] == '\'' && value_str[i + 1] == '\'') {
                        buf[j++] = '\''; // collapsed escaped quote
                        i++; // skip next
                    } else {
                        buf[j++] = value_str[i];
                    }
                ...
                buf[j] = '\0';
                val = mxCreateString(buf);
                mxFree(buf);
            } else {
                // Not quoted → use as-is
                val = mxCreateString(value_str);
            }
        }

        mxSetCell(cell, i + nkeys, val);

        // ---- Column 3: comment ----
        mxSetCell(cell, i + 2*nkeys, mxCreateString(comment));
    }
...
// Increase CFITSIO internal cache (important for .fz) % does not exist, GPT invented it?
//     fits_set_cache_size(fptr, 200 * 1024 * 1024);
//     fits_set_iobuffer(fptr, 70000);
    ...
    if (bitpix == SHORT_IMG) {
        ...
        if (bzero == 32768.0) {
            ...
            if (!use_subset) {
                // --- Full image read (faster path) ---
                long firstelem = 1;
                // mexPrintf("Hello from MEX!\n");
                
                fits_read_img(fptr, TUSHORT,
                        firstelem,
                        nelements,
                        NULL,
                        data,
