...
void mfind_bin(const float* X, mwSize N, const float* Vals, mwSize Nvals, std::vector<uint32_t>& Im, bool useOpenMP) {
    ...
    while (!converged) {
        ...
        for (mwSize i = 0; i < Nvals; ++i) {
            ...
            uint32_t newIm = static_cast<uint32_t>(std::floor(0.5f * (I1[i] + I2[i])));
            if (newIm != Im[i]) {
                Im[i] = newIm;
                converged = false;
            }
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    if (nrhs != 2) {
        mexErrMsgIdAndTxt("MATLAB:mex_mfind_bin_float:invalidNumInputs", "Two inputs required.");
    }
    if (nlhs != 1) {
        mexErrMsgIdAndTxt("MATLAB:mex_mfind_bin_float:invalidNumOutputs", "One output required.");
    }
