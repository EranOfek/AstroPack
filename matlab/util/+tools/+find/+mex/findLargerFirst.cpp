...
// mex CXXFLAGS="\$CXXFLAGS -mavx2" LDFLAGS="\$LDFLAGS" findLargerFirst.cpp

// Template function to handle both single and double precision arrays
template <typename T>
void findLargerFirst(const T* array, mwSize numElements, T value, mwIndex& index) {
    ...
    if (std::is_same<T, double>::value) {
        ...
        for (mwSize i = 0; i <= numElements - 4; i += 4) {
            ...
            __m256d mask = _mm256_cmp_pd(vec, valVec, _CMP_GT_OQ);
        ...
        for (mwSize i = 0; i <= numElements - 8; i += 8) {
            ...
            __m256 mask = _mm256_cmp_ps(vec, valVec, _CMP_GT_OQ);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    if (nrhs != 2) {
        mexErrMsgIdAndTxt("MATLAB:findLargerFirst1:invalidNumInputs", "Two input arguments required.");
    }
    if (nlhs > 1) {
        mexErrMsgIdAndTxt("MATLAB:findLargerFirst1:maxlhs", "Too many output arguments.");
    }
    ...
    if (mxGetNumberOfElements(value) != 1) {
        mexErrMsgIdAndTxt("MATLAB:findLargerFirst1:inputNotScalar", "Value must be a scalar.");
    }
    ...
    if (mxIsDouble(array) && mxIsDouble(value)) {
        ...
        findLargerFirst(arrayData, numElements, valueData, index);
        ...
        mexErrMsgIdAndTxt("MATLAB:findLargerFirst1:typeMismatch", "Array and Value must be of the same type.");
    }
