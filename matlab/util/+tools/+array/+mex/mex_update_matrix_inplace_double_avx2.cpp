...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Ensure data alignment for AVX2 usage, assuming __Type is double
    // Process in blocks of 4 doubles since each __m256d holds four doubles

    int vectorSize = 4; // number of doubles in one AVX2 register
    int numFullVectors = width_B / vectorSize;
    int remainingElements = width_B % vectorSize;

    for (int i = 0; i < height_B; ++i) {
        for (int j = 0; j < numFullVectors * vectorSize; j += vectorSize) {
            int idxA = (y1_A + i) * dimsA[0] + (x1_A + j);
            int idxB = (y1_B + i) * dimsB[0] + (x1_B + j);
            __m256d vecA = _mm256_loadu_pd(&A[idxA]);
            __m256d vecB = _mm256_loadu_pd(&B[idxB]);
            __m256d vecResult = _mm256_add_pd(vecA, vecB);
            _mm256_storeu_pd(&A[idxA], vecResult);
        }
        for (int j = numFullVectors * vectorSize; j < width_B; ++j) {
            int idxA = (y1_A + i) * dimsA[0] + (x1_A + j);
            int idxB = (y1_B + i) * dimsB[0] + (x1_B + j);
            A[idxA] += B[idxB];
        }
