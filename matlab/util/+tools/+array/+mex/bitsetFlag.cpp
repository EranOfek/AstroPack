...
// ===== AVX2 kernel for uint32: general-purpose SET =====
static void bitset_u32_avx2_set(const uint32_T* __restrict A,
                                const mxLogical* __restrict F,
                                ...
                                unsigned bit0)
{
    ...
    #endif
    {
        ...
        const bool ok_stream = ((End - Start) >= (1u<<20)) && is_aligned_32(Out + Start);

        size_t i = Start;
        size_t VecEnd = Start + (((End - Start) / 32) * 32);

        for (; i < VecEnd; i += 32)
        {
            for (int blk = 0; blk < 4; ++blk) {
                const size_t j = i + (size_t)(8 * blk);

                __m256i va  = _mm256_loadu_si256((const __m256i*)(A + j));
                __m128i vf8 = _mm_loadl_epi64((const __m128i*)(F + j));
                __m256i vf  = _mm256_cvtepu8_epi32(vf8);
                __m256i vsel = _mm256_cmpgt_epi32(vf, vZero);
                __m256i vr   = _mm256_or_si256(va, _mm256_and_si256(vsel, vBit));

                if (ok_stream) _mm256_stream_si256((__m256i*)(Out + j), vr);
                else           _mm256_storeu_si256((__m256i*)(Out + j), vr);
            }
        }

        VecEnd = Start + (((End - i) / 8) * 8) + (i - Start);
        for (; i < VecEnd; i += 8)
        {
...
// ===== AVX2 kernel for uint32: general-purpose CLEAR =====
static void bitset_u32_avx2_clear(const uint32_T* __restrict A,
                                  const mxLogical* __restrict F,
                                  uint32_T* __restrict Out,
                                  size_t N,
                                  unsigned bit0)
    ...
    #endif
    {
        ...
        for (; i < VecEnd; i += 32)
        {
            for (int blk = 0; blk < 4; ++blk) {
                ...
                __m256i vr   = _mm256_and_si256(va, _mm256_xor_si256(vAll, vmsk));
...
// ===== AVX2 kernel for uint32: all flags true, SET =====
static void bitset_u32_alltrue_set(const uint32_T* __restrict A,
                                   uint32_T* __restrict Out,
                                   size_t N,
                                   unsigned bit0)
{
    ...
    #endif
    {
        ...
        for (; i < VecEnd; i += 32) {
            for (int blk = 0; blk < 4; ++blk) {
                const size_t j = i + (size_t)(8 * blk);
                __m256i va = _mm256_loadu_si256((const __m256i*)(A + j));
                __m256i vr = _mm256_or_si256(va, vM);
                _mm256_storeu_si256((__m256i*)(Out + j), vr);
            }
        }
        ...
        const uint32_T M = (uint32_T(1u) << bit0);
        for (; i < End; ++i) Out[i] = A[i] | M;
    }
}

// ===== AVX2 kernel for uint32: all flags true, CLEAR =====
static void bitset_u32_alltrue_clear(const uint32_T* __restrict A,
                                     uint32_T* __restrict Out,
                                     size_t N,
                                     unsigned bit0)
{
    const __m256i vAll  = _mm256_set1_epi32(-1);
    const __m256i vM    = _mm256_set1_epi32((int32_T)(uint32_T(1u) << bit0));
    const __m256i vKeep = _mm256_xor_si256(vAll, vM);
    ...
    #endif
    {
        ...
        for (; i < VecEnd; i += 32) {
            for (int blk = 0; blk < 4; ++blk) {
                const size_t j = i + (size_t)(8 * blk);
                __m256i va = _mm256_loadu_si256((const __m256i*)(A + j));
                __m256i vr = _mm256_and_si256(va, vKeep);
                _mm256_storeu_si256((__m256i*)(Out + j), vr);
            }
        }
        ...
        const uint32_T Keep = ~((uint32_T)1u << bit0);
        for (; i < End; ++i) Out[i] = A[i] & Keep;
    }
}
#endif // __AVX2__
...
static void run_for_class(const mxArray* Arr, const mxArray* Flag,
                          mxArray* Out, int bit1_based, bool setOne, bool usePrescan)
{
    ...
    switch (id) {
        case mxUINT8_CLASS: {
            if (bit1_based > 8)
                mexErrMsgIdAndTxt("bitsetFlag:bit","BitNumber for uint8 in [1,8].");
        ...
        case mxUINT16_CLASS: {
            if (bit1_based > 16)
                mexErrMsgIdAndTxt("bitsetFlag:bit","BitNumber for uint16 in [1,16].");
            ...
            if (usePrescan && N >= (1u<<18)) {
                ...
                if (flags_all_zero_or_one(F, N, all_one)) {
                    if (!all_one) {
                        ...
                        if (setOne) bitset_u32_alltrue_set(a, o, N, bit0);
                        else        bitset_u32_alltrue_clear(a, o, N, bit0);
                        return;
                    }
                }
            }

            if (setOne) bitset_u32_avx2_set(a, F, o, N, bit0);
            else        bitset_u32_avx2_clear(a, F, o, N, bit0);
            #else
        ...
        case mxUINT64_CLASS: {
            if (bit1_based > 64)
                mexErrMsgIdAndTxt("bitsetFlag:bit","BitNumber for uint64 in [1,64].");
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    if (nrhs != 4 && nrhs != 5)
        mexErrMsgIdAndTxt("bitsetFlag:usage",
            "Usage: Result = bitsetFlag(Array, FlagArray, BitNumber, SetVal [, UsePrescan]);");
    if (nlhs > 1)
    ...
    if (!(b == b))
        mexErrMsgIdAndTxt("bitsetFlag:bit","BitNumber must not be NaN.");
    ...
    // Optional UsePrescan, default false
    bool usePrescan = false;
    if (nrhs >= 5) {
        const mxArray* Pre = prhs[4];
        if (mxIsComplex(Pre) || mxGetNumberOfElements(Pre) != 1)
            mexErrMsgIdAndTxt("bitsetFlag:prescan","UsePrescan must be a real or logical scalar.");
        const double pv = mxGetScalar(Pre);
        usePrescan = (pv != 0.0);
    }
    ...
    run_for_class(Arr, Flg, plhs[0], bit1, setOne, usePrescan);
}
