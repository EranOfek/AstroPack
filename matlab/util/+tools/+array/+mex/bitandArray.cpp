...
static void reduce_dim1(const T* __restrict A, mwSize M, mwSize N, T* __restrict B)
{
    ...
    for (mwIndex j = 0; j < (mwIndex)N; ++j)
    {
        ...
        if (M >= vecElems)
        {
            __m256i vacc = _mm256_cmpeq_epi8(_mm256_setzero_si256(), _mm256_setzero_si256()); // all 1s
            const size_t limit = (M / vecElems) * vecElems;
            for (; i < limit; i += vecElems)
            {
                __m256i v = _mm256_loadu_si256((const __m256i*)(col + i));
                vacc = _mm256_and_si256(vacc, v);
            }
            ...
            T vpart = std::numeric_limits<T>::max();
            for (size_t k=0; k<K; ++k) vpart &= t[k];
            acc &= vpart;
        } else {
            size_t i = 0; // fallthrough to tail
            (void)i;
        }
        // tail
        for (mwIndex i_tail = (mwIndex)((M/vecElems)*vecElems); i_tail < (mwIndex)M; ++i_tail)
            acc &= col[i_tail];
        #else
        // Scalar
...
{
    if (mxIsComplex(A)) mexErrMsgIdAndTxt("bitandArray:type","A must be real.");
    if (mxGetNumberOfDimensions(A) != 2) mexErrMsgIdAndTxt("bitandArray:ndims","A must be a 2-D matrix.");
    if (dim != 1 && dim != 2) mexErrMsgIdAndTxt("bitandArray:dim","Dim must be 1 or 2.");
    ...
    *out = mxCreateNumericArray(2, odims, cls, mxREAL);
    ...
    // Empty behavior: AND-identity is all-ones → fill with max(T)
    if (M==0 || N==0) {
        size_t outN = (size_t)odims[0]*odims[1];
        switch (cls){
          case mxUINT8_CLASS:  std::fill_n(reinterpret_cast<uint8_T* > (mxGetData(*out)), outN, std::numeric_limits<uint8_T >::max()); break;
          case mxUINT16_CLASS: std::fill_n(reinterpret_cast<uint16_T*>(mxGetData(*out)), outN, std::numeric_limits<uint16_T>::max()); break;
          case mxUINT32_CLASS: std::fill_n(reinterpret_cast<uint32_T*>(mxGetData(*out)), outN, std::numeric_limits<uint32_T>::max()); break;
          case mxUINT64_CLASS: std::fill_n(reinterpret_cast<uint64_T*>(mxGetData(*out)), outN, std::numeric_limits<uint64_T>::max()); break;
          default: mexErrMsgIdAndTxt("bitandArray:type","A must be uint8/uint16/uint32/uint64.");
        }
        return;
    }
    ...
    switch (cls)
    {
        ...
        default:
            mexErrMsgIdAndTxt("bitandArray:type","A must be uint8/uint16/uint32/uint64.");
    }
}
...
{
    if (nrhs != 2)
        mexErrMsgIdAndTxt("bitandArray:usage","Usage: B = bitandArray(A, Dim);");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("bitandArray:usage","One output only.");

    const mxArray* A = prhs[0];
    if (!mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) || mxGetNumberOfElements(prhs[1])!=1)
        mexErrMsgIdAndTxt("bitandArray:dim","Dim must be a real scalar 1 or 2.");
