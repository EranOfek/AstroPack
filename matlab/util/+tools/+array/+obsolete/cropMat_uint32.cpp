...
void cropMat(const mxArray* Array, const double CCDSEC[4], mxArray** SubMat) {
    ...
    *SubMat = mxCreateNumericMatrix(subNumRows, subNumCols, mxUINT32_CLASS, mxREAL);
    uint32_T* subMatData = static_cast<uint32_T*>(mxGetData(*SubMat));
    
    // Get the pointer to the input matrix data
    uint32_T* arrayData = static_cast<uint32_T*>(mxGetData(Array));
    
    // Copy data to the submatrix using a single loop with OpenMP
    #pragma omp parallel for
    for (mwSize col = 0; col < subNumCols; ++col) {
        uint32_T* subColStart = subMatData + col * subNumRows;
        uint32_T* arrayColStart = arrayData + (Jmin + col) * numRows + Imin;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!mxIsUint32(Array)) {
        mexErrMsgIdAndTxt("MATLAB:cropMat:invalidArray", "Input Array must be a uint32 matrix.");
    }
