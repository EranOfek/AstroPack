...
%             Coo = [X, Y];
        ...
        function [Obj, FitResult] = fitPar(Obj, InputValues, ObservedValues, Args)
            ...
            arguments
                ...
                % Scale estimator for the 'median' clipping method:
                %   'mad_std' - 1.4826 * MAD (robust; default for back-compat)
                %   'std'     - sample standard deviation (matches astropy
                %               sigma_clip default; used by LAST_NormLin's
                ...
                if ~ConvergedSigmaClip
                    ...
                    if Args.SigmaClip && Iter > 1 && ~ConvergedSigmaClip
                        ...
                        [OutlierMask, ClipInfo] = tools.math.stat.sigmaClip(...
                            ...
                            'Method', Args.SigmaClipMethod, ...
                            'StdFunc', Args.SigmaStdFunc, ...
                            'Errors', MagErr);
        ...
        function [Obj, FitResult] = fitMultiStage(Obj, InputValues, ObservedValues, Args)
            ...
            for IStage = 1:NumStages
                ...
                % SigmaStdFunc: per-stage robust/sample std selector for the
                % 'median' clipping method. Defaults to 'mad_std' (robust) for
                % back-compat. LAST_NormLin sets 'std' at the field-correction
                % stage to match astropy's sample-std default (Simone parity).
                if isfield(Stage, 'SigmaStdFunc') && ~isempty(Stage.SigmaStdFunc)
                    SigmaStdFunc = Stage.SigmaStdFunc;
                else
                    SigmaStdFunc = 'mad_std';
                end
