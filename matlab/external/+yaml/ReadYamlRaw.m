function result = ReadYamlRaw(filename, verbose, nosuchfileaction, treatasdata)
    ...
    try
		...
		% For deployed application, look for jar file in in binary file, because addpath() etc. are not supported
        if isdeployed
            dp = ['snakeyaml-1.9.jar'];
        else
            dp = [pth filesep 'external' filesep 'snakeyaml-1.9.jar'];
        end
		
        %fprintf('ReadYamlRaw: pth: %s\n', pth);
		%fprintf('ReadYamlRaw: javaaddpath: %s\n', dp);
