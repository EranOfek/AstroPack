% This demonstrates that bilinear interpolation and bicubic interpolation produce different results.
% The bilinear results are slightly inferior.
...
% This demonstration shows that the surfaces are equivalent when they have the same smoothing, regardless of the x,y
% grid density.
Smoothness = 0.003;
...
ylabel('y');
zlabel('z');
title({['Regularized output surface generated with Bicubic Interpolation']; ' Original, scattered input points in blue'});
...
z = RegularizeData3D(InputPoints(:, 1), InputPoints(:, 2), InputPoints(:, 3), x, y, 'interp', 'bilinear', 'smoothness', Smoothness);
...
ylabel('y');
zlabel('z');
title({['Regularized Output Surface generated with Bilinear Interpolation - Not quite right']; ' Original, scattered input points in blue'});
set(get(gca,'XLabel'),'FontSize', 12)
set(get(gca,'YLabel'),'FontSize', 12)
set(get(gca, 'Title'), 'FontSize', 12)
axis([0, 4, 0, 4, 0, 1.1]);