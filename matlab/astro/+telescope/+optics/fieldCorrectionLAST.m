function Result = fieldCorrectionLAST(Coords, ParamMatrix, Args)
    % LAST field-dependent magnitude corrections using Chebyshev polynomials
    ...
    %                   Column 10: kxy (Cross term kxy * T1(x)*T1(y) - linear formulation)
    %                   Default is zeros(1,10) (no field correction).
    %          * ...,key,val,...
    %            'Return' - External cached result.
    %                   Default is [].
    %            'UsePersistentCache' - Use persistent cache.
    %                   Default is true.
    %            'GetArgNames' - Return parameter names structure.
    %                   Default is false.
    ...
    % Example: Coords = [100, 100; 863, 863; 1600, 1600];
    %          Params = [0.1, 0.02, 0.01, 0, 0, 0.02, 0.01, 0, 0, 0.005];
    %          Corrections = telescope.optics.fieldCorrectionLAST(Coords, Params);
    ...
    for i = 1:NumParamSets
        ...
        % Linear formulation (superior to kxy² - allows negative coefficients)
        Cross_term = kxy * T_x(:, 2) .* T_y(:, 2);
