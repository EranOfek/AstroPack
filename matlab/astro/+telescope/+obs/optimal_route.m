function [OptRoute, MinDist] = optimal_route(RA, Dec, Args)
    % Optimize order of visits to a set of sky points in terms of distance 
    %     Based on a genetic algorithm 
    % Input  : - RA [deg]
    %          - Dec [deg]
    %          * ...,key,val,... 
    %          'NGen' - population size (should be divisible by 4) 
    %          'NIt'  - number of iterations
    %          'ShowProgress' - logical
    %          'ShowResult'   - plots with final result and distance matrix
    % Output : - optimal order of pointings
    %          - optimal distance (sum of repointings) in [deg]
    % Author : A.M. Krassilchtchikov (2024 Sep) 
    % Example: RA = 360.*rand(20,1); Dec = 180.*rand(20,1)-90.; 
    %          [OptRoute, MinDist] = telescope.obs.optimal_route(RA, Dec,'NIt', 20, 'ShowResult', true)
    arguments
        RA
        Dec
        Args.NGen         =  60;
...
if showResult
    ...
    title('Sky Positions'); xlabel 'RA, deg'; ylabel 'Dec, deg';
    subplot(2,2,2);
