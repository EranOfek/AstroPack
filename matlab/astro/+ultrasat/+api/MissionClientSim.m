    ...
    methods
        ...
        function response = login(obj, UserName, Password)
            ...
            if isempty(user)
                ...
                fwrite(fid, jsonencode(currentUser), 'char');
                fclose(fid);
        ...
        function response = logout(obj, UserName)
            ...
            % Clear current user
            fid = fopen(currentUserFile, 'w');
            fwrite(fid, jsonencode(struct('UserName', '', 'Role', '')), 'char');
            fclose(fid);
        ...
        function response = savePlan(obj)
            ...
            fwrite(fid, jsonencode(planStruct), 'char');
            fclose(fid);
