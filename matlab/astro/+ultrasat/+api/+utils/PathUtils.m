...
classdef PathUtils < handle
    ...
    methods (Static)
        ...
        function folderPath = getGlobalDataFolder(moduleName, subFolder)
            ...
            folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', '_global', moduleName, subFolder), filesep];
        end
        ...
        function folderPath = getNamespaceDataFolder(moduleName, subFolder, options)
            ...
            nsId = ultrasat.api.utils.PathUtils.resolveNamespaceId(options.NamespaceId);
            folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', nsId, moduleName, subFolder), filesep];
        end
        ...
        function folderPath = getGlobalDailyDataFolder(moduleName, subFolder, options)
            ...
            dt = ultrasat.api.utils.PathUtils.resolveDateTime(options.DT);
            % CHANGE: datestr instead of string()
            dateSubFolder = datestr(dt, 'yyyy-mm-dd');

            % CHANGE: isempty() check instead of == ""
            if isempty(subFolder)
                folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', '_global', moduleName, dateSubFolder), filesep];
            else
                folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', '_global', moduleName, subFolder, dateSubFolder), filesep];
            end
        end
        ...
        function folderPath = getNamespaceDailyDataFolder(moduleName, subFolder, options)
            ...
            if isempty(subFolder)
                folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', nsId, moduleName, dateSubFolder), filesep];
            else
                folderPath = [fullfile(ultrasat.api.utils.PathUtils.BasePath, 'data', 'namespaces', nsId, moduleName, subFolder, dateSubFolder), filesep];
            end
        end
        ...
        function filePath = getGlobalDataFilename(moduleName, subFolder, fileName)
            ...
            folder = ultrasat.api.utils.PathUtils.getGlobalDataFolder(moduleName, subFolder);
        ...
        function filePath = getNamespaceDataFilename(moduleName, subFolder, fileName, options)
            ...
            folder = ultrasat.api.utils.PathUtils.getNamespaceDataFolder(moduleName, subFolder, 'NamespaceId', options.NamespaceId);
            filePath = fullfile(folder, fileName);
        end
        ...
        function filePath = getGlobalDailyDataFilename(moduleName, subFolder, fileName, options)
            ...
            dt = ultrasat.api.utils.PathUtils.resolveDateTime(options.DT);
            folder = ultrasat.api.utils.PathUtils.getGlobalDailyDataFolder(moduleName, subFolder, 'DT', dt);
        ...
        function filePath = getNamespaceDailyDataFilename(moduleName, subFolder, fileName, options)
            ...
            dt = ultrasat.api.utils.PathUtils.resolveDateTime(options.DT);
            folder = ultrasat.api.utils.PathUtils.getNamespaceDailyDataFolder(moduleName, subFolder, 'DT', dt, 'NamespaceId', options.NamespaceId);
            if options.IncludeTimestampInFilename
                timestamp = datestr(dt, 'yyyy-mm-dd-HH-MM-SS');
                fileName = [timestamp, '-', fileName];
            end
        ...
        function filePath = getGlobalLogFilename(moduleName, fileName, options)
            ...
            dt = ultrasat.api.utils.PathUtils.resolveDateTime(options.DT);
            logSubPath = ultrasat.api.utils.PathUtils.getLogSubfolderFilename(fileName, dt);
            filePath = fullfile(ultrasat.api.utils.PathUtils.BasePath, 'log', 'namespaces', '_global', moduleName, logSubPath);
        end
        ...
        function filePath = getNamespaceLogFilename(moduleName, fileName, options)
            ...
            nsId = ultrasat.api.utils.PathUtils.resolveNamespaceId(options.NamespaceId);
            logSubPath = ultrasat.api.utils.PathUtils.getLogSubfolderFilename(fileName, dt);
            filePath = fullfile(ultrasat.api.utils.PathUtils.BasePath, 'log', 'namespaces', nsId, moduleName, logSubPath);
        end
        ...
        function setBasePath(basePath)
            ...
            ultrasat.api.utils.PathUtils.BasePath(path);
        end
        ...
        function setNamespaceId(Id)
            ...
            ultrasat.api.utils.PathUtils.NamespaceId = Id;
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function nsId = resolveNamespaceId(providedNsId)
            ...
            if ~isempty(providedNsId)
                nsId = providedNsId;
            elseif ~isempty(ultrasat.api.utils.PathUtils.NamespaceId)
                nsId = ultrasat.api.utils.PathUtils.NamespaceId;
            else
