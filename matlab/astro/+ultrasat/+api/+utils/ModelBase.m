...
classdef ModelBase
    ...
    methods (Static)
        ...
        function jsonStr = struct2json(Data)
            % Converts the Data property to a JSON string, converting datetime fields
            jsonReadyData = ultrasat.api.utils.ModelBase.convertDatetimeToString(Data);
        ...
        function jsonStruct = json2struct(jsonStr)
            ...
            jsonStruct = ultrasat.api.utils.ModelBase.convertStringToDatetime(decodedStruct);
        end
        ...
        function data = convertDatetimeToString(data)
            ...
            if iscell(data)
                for i = 1:numel(data)
                    if isstruct(data{i}) || iscell(data{i})
                        data{i} = ultrasat.api.utils.ModelBase.convertDatetimeToString(data{i});
                    elseif isdatetime(data{i}) && ~isempty(data{i})
                        data{i} = ultrasat.api.utils.ModelBase.isoFormat(data{i});
                    end
                end
                return
            end
            ...
            if isstruct(data)
                ...
                if numel(data) > 1
                    for k = 1:numel(data)
                        data(k) = ultrasat.api.utils.ModelBase.convertDatetimeToString(data(k));
                    end
                    return
                end
                ...
                for i = 1:numel(fields)
                    ...
                    if isdatetime(value) && ~isempty(value)
                        data.(fieldName) = ultrasat.api.utils.ModelBase.isoFormat(value);

                    elseif isstruct(value) || iscell(value)
                        data.(fieldName) = ultrasat.api.utils.ModelBase.convertDatetimeToString(value);
                    end
                end
            end
        end
                ...
                for i = 1:numel(fields)
                    ...
                    elseif isstruct(value)
                        ...
                        elseif numel(value) > 1
                            ...
                            for j = 1:numel(value)
                                value(j) = ultrasat.api.utils.ModelBase.convertStringToDatetime(value(j));
                            end
                            data.(fieldName) = value;
                        else
                            % Handle single struct
                            data.(fieldName) = ultrasat.api.utils.ModelBase.convertStringToDatetime(value);
                        end
                    end
                end
            end
