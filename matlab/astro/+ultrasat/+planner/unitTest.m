% unitTest - Integration smoke tests for ultrasat.planner.uplanner.
%
% Exercises end-to-end plan construction and editing for each supported survey
% type. Intended as a manual or CI smoke check, not a formal matlab.unittest suite.
%
% Survey parts (select via Args.Parts):
%   HCS    - build a minimal High Cadence Survey plan
%   LCS    - buildLCS1, buildLCS, mission-approved plan retrieval, plan edits
%   TOO    - minimal and probability-map Target of Opportunity plans
%   DDT    - Director's Discretionary Time target scheduling
%   AllSS  - two-stage All-Sky Survey build (optional; off by default)
%
% Dependencies:
%   LCS requires HCS when using retrieveMissionApprovedPlan with upHCS.Plan.
%   Include 'HCS' in Args.Parts whenever 'LCS' is run (default Parts do this).
%
% Usage:
%   ultrasat.planner.unitTest()
%   ultrasat.planner.unitTest('Parts', {'LCS'}, 'Verbose', false)
%
% Returns:
%   Result - true if all selected parts complete without failure; false otherwise
%            (e.g. LCS planSelfConsistencyCheck fails).

function Result = unitTest(Args)
    % Integration smoke tests for ultrasat.planner.uplanner.
    arguments
        Args.Verbose   = true;
...
% ================================== Test LCS plan ==================================
function Result = testLCS(Args, upHCS)
    Result = true;
    if Args.Verbose
