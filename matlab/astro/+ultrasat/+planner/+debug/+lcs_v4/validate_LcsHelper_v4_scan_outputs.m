function validate_LcsHelper_v4_scan_outputs(varargin)
% Validate all v4 scan CSV output folders for one year.
%
% This validator reads existing scan outputs. It does not rebuild plans.
% Per-date reports are written inside each yyyy-mm-dd/success folder, and a
% yearly index is written in the scan root.
    ...
    fprintf('validate_LcsHelper_v4_scan_outputs\n');
    fprintf('ScanDir: %s\n', ScanDir);
...
function [Passed, FailedCount, WarningCount, Lines] = finish_report(Checks, Warnings)
    ...
    Lines = {
        'validate_LcsHelper_v4_scan_outputs report'
