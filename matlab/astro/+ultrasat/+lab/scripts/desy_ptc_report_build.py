...
w('## 8. Shape of the photon-transfer curve: non-linearity and the low-signal excess')
w('')
w('Sections 4–7 fit the variance–mean relation in a fixed window (300–2500 ADU) and quote one gain per die. This section examines the shape of that relation over the whole unsaturated range, for the three deck die-runs, and separates what is detector response from what is read noise or averaging.')
w('')
w('### 8.1 Straight-line fits above 500 ADU')
w('')
w('Weighted fits of Var vs Mean (temporal estimator, 10 000 pixels, 3 frames per step, so SE(Var)/Var = 1 %):')
w('')
w('| Die / run | Range [ADU] | n | Slope [ADU/e⁻] | Intercept [ADU²] | Intercept / SE | χ²/dof |')
w('|---|---|---|---|---|---|---|')
w('| W04_D07 r31 | 500–13 000 | 15 | 1.0105 ± 0.0036 | 116 ± 6 | 19 σ | 458/13 |')
w('| W04_D07 r31 | 500–2 500 | 5 | 1.0971 ± 0.0107 | 57 ± 10 | 5.8 σ | 1/3 |')
w('| W08_D02 r31 | 500–13 000 | 14 | 0.9753 ± 0.0039 | 169 ± 9 | 19 σ | 530/12 |')
w('| W08_D02 r31 | 500–2 500 | 4 | 1.0732 ± 0.0142 | 85 ± 17 | 5.1 σ | 1/2 |')
w('| W04_D07 r32 | 500–13 000 | 15 | 1.0067 ± 0.0035 | 93 ± 6 | 17 σ | 443/13 |')
w('| W04_D07 r32 | 500–2 500 | 5 | 1.0853 ± 0.0102 | 40 ± 9 | 4.4 σ | 7/3 |')
w('')
w('The wide-range fits have χ²/dof = 34–44: above ~2.5 kADU the relation is **not** a straight line, so their slope and intercept have no physical meaning (they differ from the narrow-range values by 8 % and a factor 2). The narrow-range fits are good straight lines, and in all three cases the intercept is **not consistent with zero** (4–6 σ).')
w('')
w('The intercept is not read noise: the read-noise variance is only 5.3–5.5 ADU², i.e. 10–20× smaller. It is the shot noise of the charge that is collected but not read out. What the pixel delivers is S = collected − T, where T is the threshold of §2 item 6 (T = DC·t − I_B = 92 / 138 / 31 ADU for the three die-runs), while the Poisson noise belongs to the collected charge S + T; hence Var = g·(S + T), i.e. an intercept g·T = 100 / 148 / 34 ADU². Measured: 57 / 85 / 40 ADU². The run-32 value (small threshold) matches; the two AV runs come out ~1.7× below the prediction, i.e. the charge held below the threshold does not carry its full Poisson noise.')
w('')
for tag, cap in [('W04_D07_run31_semilog.png', 'W04_D07, run 31'),
                 ('W08_D02_run31_semilog.png', 'W08_D02, run 31'),
                 ('W04_D07_run32_semilog.png', 'W04_D07, run 32')]:
    w('![%s](%s)' % (cap, tag)); w('')
    w('*%s. Left: variance vs mean (log–log) with the two fits and the zero-intercept line. Middle: Var/(Mean·Gain) for Gain = 1.02 … 1.10. Right: the response deviation from the line fitted at 1–2.5 kADU (purple), the differential slope d S/d int normalised to its low-signal value (orange), and √(Var/(g·(Mean+T))) − 1 (green), i.e. what the PTC implies for the differential response.*' % cap); w('')
w('### 8.2 The dip is the response non-linearity')
w('')
w('For a non-linear response S = F(Q) the variance is Var = F′(Q)²·g·Q, so the PTC measures the *square* of the differential response, while the ladder measures the response itself. The right-hand panels above show the two independently: above 2.5 kADU the green curve (from the variance) lies on top of the orange one (from the response ladder) in all three die-runs. The dip in the PTC is therefore not a noise artefact — it is the non-linearity.')
w('')
w('| | W04_D07 r31 | W08_D02 r31 | W04_D07 r32 |')
w('|---|---|---|---|')
w('| Linear to 0.5 % up to | 2.9 kADU | 2.8 kADU | 2.9 kADU |')
w('| Integral non-linearity at 5–12 kADU | −5.2 … −5.5 % | −5.4 … −5.9 % | −4.3 … −5.0 % |')
w('| Differential slope, minimum | 0.892 at 4.7 kADU | 0.884 at 4.5 kADU | 0.901 at 4.7 kADU |')
w('| Integral non-linearity at ~14 kADU | −21 % | −14 % | −17 % |')
w('| … at ~14.5–15 kADU | −33 % | −29 % | −30 % |')
w('')
w('So the response is linear to better than 0.5 % below ~3 kADU — the regime of the thresholds and of faint sources — loses ≈5 % between 3 and 5 kADU and keeps that offset up to 12 kADU, and compresses by 20–30 % in the last 2 kADU before ADC saturation. The effect is identical in the two flavours (W04 = Fl.6, W08 = Fl.2) and in both bias settings, i.e. it is a fixed, calibratable property of the readout chain.')
w('')
w('### 8.3 Read-noise subtraction, dark ladder, robust variance')
w('')
w('![RN-corrected panels](rn_corrected_panels.png)')
w('')
w('*(Var − RN²)/(Mean·Gain) for Gain = 1.02 … 1.10; top row light ladders, bottom row dark ladders. Per-step variance = median over the pixels of the per-pixel temporal variance divided by ln 2 (unbiased for 3 frames, robust against particle hits); RN² from the ZE frames with the same estimator.*')
w('')
w('- Subtracting RN² changes the ratio by 0.04 at the lowest light step, 0.07 at the lowest dark step and < 0.001 above 5 kADU: **the low-signal excess is not read noise**.')
w('- The plain mean over pixels of the per-pixel variance is not robust — a single particle hit adds ≈800 ADU² — and two steps of the run-32 dark ladder came out ×8 and ×1.8 too high. The median/ln 2 estimator agrees with the mean to 0.5 % on all clean steps and removes those outliers.')
w('- The **dark ladder reproduces the light ladder**: the same excess below ~300 ADU and the same fall-off from 2–3 kADU (down to 0.85 and 0.76 at the last dark steps). The non-linearity is in the collected charge, not in the illumination, so it is not an LED-stability or flat-field artefact.')
w('')
w('### 8.4 Single pixels, without any spatial averaging')
w('')
w('![single-pixel panels](perpixel_panels_zoom.png)')
w('')
w('*The same ratio formed inside each pixel — its own mean, its own temporal variance and its own read noise — and combined over the 10 000 pixels only at the end, by the median / ln 2. Grey: six individual pixels.*')
w('')
w('The per-pixel curves reproduce the region curves (light: 1.15–1.21 at ~130 ADU, ≈1.10 at 1–2.5 kADU, 0.87–0.88 at the dip, 1.01–1.05 above 7 kADU), so none of the structure is an artefact of pairing one pixel’s variance with the region’s mean. A single pixel on its own is useless here: with 3 frames its variance has 2 degrees of freedom and ~100 % scatter (grey traces); 10 % per point would need ~200 frames per step.')
w('')
w('Pixel-to-pixel uniformity of the gain, tested on the five bright steps in 300–2500 ADU: the observed scatter of the per-pixel ratio is 0.447 / 0.453 / 0.454 against 0.447 expected from the 2-dof noise alone, and the split-half correlation (odd steps vs even steps of the same pixel) is −0.005 / +0.007 / +0.002. The gain is therefore uniform to better than ~8 % (1 σ) and no pixel-specific structure is detectable. For comparison the response non-uniformity, which is measured on the means and hence far more precisely, is 1.3–1.4 %.')
w('')
w('### 8.5 Scatter across pixels')
w('')
w('The same per-pixel data answer the complementary question: how much do the pixels of the region differ from each other, and is that difference random or structured? Each pixel contributes its own mean signal at every step; the scatter of those means over the 10 000 pixels is corrected for the temporal noise of a 3-frame average (σ²_fixed = σ²_observed − ⟨V_i⟩/3).')
w('')
w('![pixel scatter](pixel_scatter.png)')
w('')
w('*W04_D07, run 31. Top: the flat field (light ladder at 11.6 kADU), the dark-current map (600 s) and the read-noise map, all for the DESY 100 × 100 region. Bottom: the pixel distributions, the fixed-pattern scatter versus signal for both ladders, and the spatial correlation of the two patterns.*')
w('')
w('| Quantity | W04_D07 r31 | W08_D02 r31 | W04_D07 r32 |')
w('|---|---|---|---|')
w('| Response non-uniformity (PRNU, noise-corrected, ~11 kADU) | 0.39 % | 0.42 % | 0.41 % |')
w('| Additive fixed pattern (fit σ² = a² + (b·S)² below 2.6 kADU) | a = 8.1 ADU, b = 0.43 % | a = 9.5 ADU, b = 0.52 % | a = 7.3 ADU, b = 0.56 % |')
w('| Dark-current non-uniformity (DSNU) | 5.6 % | 5.8 % | 7.5 % (faint ladder) |')
w('| Read noise: median / fraction > 2× median / > 3× | 1.92 ADU / 5.1 % / 0.7 % | 1.95 ADU / 8.6 % / 3.1 % | 1.92 ADU / 5.1 % / 0.8 % |')
w('| Response even / odd raw columns | +0.43 % | +0.22 % | +0.15 % |')
w('| Dark current even / odd raw columns | +1.3 % | +0.9 % | +1.0 % |')
w('')
w('- **Two different fixed patterns.** The illuminated scatter is the sum of an additive term of 7–10 ADU (≈7–9 e⁻, independent of signal) and a multiplicative term of 0.4–0.6 %; the additive term dominates below ~2 kADU. The dark scatter is purely multiplicative at 5.6–5.8 % of the accumulated dark signal, i.e. more than ten times the PRNU. Dark current, not photo-response, is what makes pixels differ.')
w('- **Different spatial character.** The flat field is white — the lag-1 correlation between neighbouring pixels is −0.007 and stays below 0.02 out to 15 pixels. The dark-current map is strongly correlated: 0.71 at lag 1, 0.27 at lag 3, 0.11 at lag 8, i.e. a coherence length of ~3 pixels, and the map shows blobs rather than isolated hot pixels. A flat-field correction therefore cannot remove the dark pattern and vice versa; the two maps are uncorrelated with each other (r = 0.04) and with the read-noise map (r ≤ 0.02).')
w('- **Read noise has a tail.** The typical pixel is at 1.92 ADU, but 5 % (W08_D02: 8.6 %) of the pixels are noisier than twice that, and 0.2–0.3 % exceed 5×. This tail is what makes the rms read noise (2.3–2.7 ADU) exceed the median, as noted in §3, and it is not correlated with response or dark current.')
w('- **The non-linearity itself varies between pixels.** In the middle panel the light-ladder scatter departs from the a ⊕ b·S law exactly where the response kinks: 13 ADU at 2.4 kADU but 34 ADU at 4.7 kADU, an excess of ≈26 ADU ≈ 0.55 % of the signal. The ≈5 % loss of §8.2 therefore varies by about ±11 % of itself from pixel to pixel — small, but it means a single global non-linearity curve leaves a ~0.5 % residual at 4–5 kADU.')
w('- The parity effect of §6 appears in both patterns: the even raw columns have a 0.15–0.43 % higher response and a 0.9–1.3 % higher dark current, and at the lowest light step their signal is 1.7–1.9 ADU lower (the additive term). Splitting the region by parity does not reduce the within-group scatter (9.57 and 9.64 ADU against 9.65 ADU for all pixels), so parity is a small offset on top of an otherwise unstructured pattern.')
w('')
w('## 9. Caveats')
w('')
