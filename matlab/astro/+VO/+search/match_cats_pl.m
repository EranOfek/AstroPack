...
Res = tools.struct.struct_def({'IndRef','IndCat','Dist','PA','Num'},0,1);
    
% binary search on sorted catalog
% search simultanously all ref sources
Iupp = tools.find.mfind_bin(Cat(:,InPar.ColDec),RefCat(:,InPar.ColRefDec).'+Radius);
Ilow = tools.find.mfind_bin(Cat(:,InPar.ColDec),RefCat(:,InPar.ColRefDec).'-Radius);
...
for Icr=1:1:Nc
    ...
    if (InPar.CalcPA)
        [Dist,PA]  = tools.math.geometry.plane_dist(Cat(Icat,InPar.ColRA),...
                                             Cat(Icat,InPar.ColDec),...
                                             RefCat(Iref,InPar.ColRefRA),...
                                             RefCat(Iref,InPar.ColRefDec));
    else
        
        [Dist]  = tools.math.geometry.plane_dist(Cat(Icat,InPar.ColRA),...
                                             Cat(Icat,InPar.ColDec),...
                                             RefCat(Iref,InPar.ColRefRA),...
                                             RefCat(Iref,InPar.ColRefDec));
    ...
    if (Vec.Nfound(Iref)>0)
        ...
        if (nargout>1)
            ...
            if (InPar.CalcPA)
                [~,Res(K).PA]  = tools.math.geometry.plane_dist(Cat(Iref,InPar.ColRA),...
