...
if (~isempty(Cat))
    ...
    switch lower(InPar.SearchMethod)
        ...
        case 'binm'
            Ind1 = tools.find.mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'-InPar.SearchRad);
            Ind2 = tools.find.mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'+InPar.SearchRad)+1;
       ...
       case 'binms'
            switch lower(InPar.CooType)
                case 'sphere'
                    ...
                    Ind1 = tools.find.mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'-InPar.SearchRad);
                    Ind2 = tools.find.mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'+InPar.SearchRad);
             ...
             switch lower(InPar.CooType)
                case 'sphere'
                    ...
                    for Is=1:1:Nin
                        ...
                        %Ireg = (Ind1(Is):1:Ind2(Is));
                        %Ireg = (Is+1:1:Ind2(Is));
                        Ireg = tools.array.delete_ind(IndReg,Is);
