...
% The interface functions in the data directory are auto generated by
% prep_data_dir, which embeds the absolute path of the machine they were
% created on (e.g. /raid/eran/matlab/data/+cats/...). If that path does not
% exist here, rebuild it under this installation's data directory, so that
% the same data files work for every user without being regenerated.
if ~isfolder(Path)
    DataPath = tools.os.getAstroPackDataPath();
    if ~isempty(DataPath)
        % locate the '/data/' element, using either separator - the stored
        % path may have come from a different platform
        Ind = max([strfind(Path,'/data/'), strfind(Path,'\data\')]);
        if ~isempty(Ind)
            Relative  = strrep(Path(Ind+6:end), '\', '/');
            LocalPath = fullfile(DataPath, Relative);
