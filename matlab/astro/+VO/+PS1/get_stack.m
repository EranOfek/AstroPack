...
for Ilink=1:1:Nlink
    ...
    % full images
    IndFull = ~tools.cell.isempty_cell(strfind(List,BaseURL));
    [LinkFull{Ilink,:}] = deal(List{IndFull});
    
    % cut images
    %IndCut = ~tools.cell.isempty_cell(strfind(List,'http://ps1images.stsci.edu/cgi-bin/fitscut'));
    IndCut = ~tools.cell.isempty_cell(strfind(List,'cutout_rings'));
