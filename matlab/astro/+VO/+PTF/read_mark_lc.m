...
FID = fopen(File,'r');
line=io.files.file2str(FID,'cell');
fclose(FID);
hdrlines=find(tools.cell.isempty_cell(strfind(line,'MJD counts'))==0);
if isempty(hdrlines)
...
Lup = astro.spec.luptitude(C{2},1,B);
...
if (~isempty(BinVec))
   ...
   for Ibin=1:1:Nbin
     switch Ncol
      ...
      [Flux,FluxErr] = tools.math.stat.wmean(LC(Isel,5:6));
      ...
      Nsel  = length(Isel);
      if (Nsel>1)
          MagStD  = tools.math.stat.bootstrap_std(LC(Isel,2),BootStrapFun,Nsim.*Nsel);
          FluxStD = tools.math.stat.bootstrap_std(LC(Isel,5),BootStrapFun,Nsim.*Nsel);
      else
          MagStD  = NaN;
          FluxStD = NaN;
      end
