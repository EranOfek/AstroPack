...
%
%   % NEDz (NED objects with redshift) -- single all-sky HDF5 dump:
%   % Step 1: Async TAP query in Dec bands writes ~/tmp/NED/NEDz_dump.hdf5
%   %         with /data attributes MinDec/MaxDec/MinRA/MaxRA in radians,
%   %         columns [RA(rad) Dec(rad) z zunc PretypeCode].
%   %         See ~/tmp/NEDz/buildNEDz.m for the dump script.
%   % Step 2: Build HTM catalog from the single all-sky dump file
%   VO.prep.buildHTMfromFiles(...
%       'CatName',     'NEDz', ...
%       'SourceDir',   '/home/dana/tmp/NED', ...
%       'FilePattern', 'NEDz_dump.hdf5', ...
%       'TargetDir',   '/home/dana/tmp/NEDz', ...
%       'HTM_Level',   7, ...
%       'CoorUnits',   'rad', ...
%       'PostReadFun', @(F) h5read(F, '/data'), ...
%       'ColNames',    {'RA','Dec','z','zunc','PretypeCode'}, ...
%       'ColUnits',    {'rad','rad','','',''}, ...
%       'NcatInFile',  100, ...
%       'IndStep',     30, ...
%       'ProcessMode', 'perfile');
