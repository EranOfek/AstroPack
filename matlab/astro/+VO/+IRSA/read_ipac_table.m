...
switch lower(FileType)
    case 'str'
        CellLines = tools.string.strlines2cell(File);
    case 'file'
        % do nothing
        CellLines = io.files.file2str(File,'cell');
    otherwise
...
%Table.Cat = tools.string.read_formatted(File,Format,'comment',{'|','\'}); 
Table.Cat = tools.string.read_str_formatted(CellLines,Format,'comment',{'|','\'});
