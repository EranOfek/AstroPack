function Result = unitTest()
    ...
    if InpopExist
        ...
        %% Test: celestial.ephem.ephemMultiObj for single object
        ...
        %% Test: celestial.ephem.ephemMultiObj for multiple objects
        OrbEl=celestial.OrbitalEl.loadSolarSystem('num',[9801:9900]);
        JD = OrbEl.Epoch(1)+3000;  % integrate to epoch+1000 days
        Result = celestial.ephem.ephemMultiObj(OrbEl, JD, 'INPOP',IN);
        % compare to JPL
        [T1] = celestial.SolarSys.getJPL_ephem('9801;','EPHEM_TYPE','OBSERVER','TimeScale','TT','StartTime',JD,'StopTime',JD+0.5); 
        ...
        %[max(abs(Resid1))] 
        if any(abs(Resid1)>0.2,'all') || any(abs(Resid2)>0.2,'all')
            max(abs(Resid1),[],'all')
            max(abs(Resid2),[],'all')
            error('Error in orbital integration - celestial.ephem.ephemMultiObj - residuals larger than 0.1 arcsec');
        end
        ...
        %IN.populateTables('all','TimeSpan',[OrbEl.Epoch(1)-10init0, JD+100], 'MaxOrder',5);
