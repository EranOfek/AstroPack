function [Res,RMS]=polar_alignment(Meas_H, Meas_D, Measured_Dt_dt, Measured_Rt_dt, Phi, Args)
    ...
    %            Default is 30.052984./(180./pi).
    %          * ...,key,val,...
    %            'Plot' - Plot rms vs. beta and psi. Default is false.
    %            'UseOnlyDec' - Use only declination motion.
    %                   Default is false.
    %            'Step' - Scanning step (deg). Default is 0.005.
    %            'MaxScan' - Max scanning range (deg). Default is 1.5.
    ...
    arguments
        ...
        Measured_Dt_dt
        Measured_Rt_dt
        Phi               = 30.052984./(180./pi);

        Args.Plot         = false;
        Args.UseOnlyDec   = false;
        Args.Step         = 0.005;  % [deg]
        Args.MaxScan      = 1.5;    % [deg]
    end
    ...
    if UsePsi
        D_Beta  = [0.0:Step:Args.MaxScan]'./RAD;
        D_Psi   = [-180:2:180]'./RAD;
        Nbeta   = numel(D_Beta);
        Npsi    = numel(D_Psi);
        ...
        D_DAz   = (-Args.MaxScan:Args.Step:Args.MaxScan)'./RAD;
        D_DAlt  = (-Args.MaxScan:Args.Step:Args.MaxScan)'./RAD;
        N_DAz   = numel(D_DAz);
        N_DAlt  = numel(D_DAlt);
