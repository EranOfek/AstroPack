...
D = (297.85036 + 445267.111480.*T - 0.0019142.*T.*T + T.*T.*T./189474)./RAD;

% Mean anomaly of the Sun (Earth):
M = (357.52772 + 35999.050340.*T - 0.0001603.*T.*T - T.*T.*T./300000)./RAD;

% Mean anomaly of the Moon:
Mt = (134.96298 + 477198.867398.*T + 0.0086972.*T.*T + T.*T.*T./56250)./RAD;

% Moon's argument of latitude
F = (93.27191 + 483202.017538.*T - 0.0036825.*T.*T + T.*T.*T./327270)./RAD;

% Longitude of ascending node of the Moon's mean orbit
Om = (125.04452 - 1934.136261.*T + 0.0020708.*T.*T + T.*T.*T./450000)./RAD;

% nutation in longitude [ 0."0001] - nutation in obliquity [ 0."0001]
% lm ls F D Om   period psi                   eps
% Mt M F D Om
Table=[  0  0  0  0  1 -6798.4 -171996.0    -174.2   92025.0       8.9     
...
DLon = bsxfun(@plus,Table(:,7),Table(:,8)*T).*sin(Table(:,1)*Mt + Table(:,2)*M + Table(:,3)*F + Table(:,4)*D + Table(:,5)*Om);

DObl = bsxfun(@plus,Table(:,9),Table(:,10)*T).*cos(Table(:,1)*Mt + Table(:,2)*M + Table(:,3)*F + Table(:,4)*D + Table(:,5)*Om);

% convert to arcsec
DLon = sum(DLon).'.*0.0001; %./(3600.*RAD);
DObl = sum(DObl).'.*0.0001; %./(3600.*RAD);
switch lower(Units)
    case 'rad'
        ConvFactor = 1./(RAD.*3600);
    case 'deg'
        ConvFactor = 1./RAD;
    case 'as'
        ConvFactor = 1;
