...
%
% BUGS: https://github.com/EranOfek/AstroPack/issues/281 
%c
...
%                                        @celestial.INPOP.chebyshevFun15};
    ...
    methods (Static)  % transformations
        ...
        function [CooJ2000, RotM] = ecliptic_2eqJ2000(CooEcl)
            % Rotate [X;Y;Z] Ecliptic coordinates to Equatorial J2000 coordinates.
            % Input  : - A 3xN matrix of positions [X;Y;Z] in equatorial J2000.
            % Output : - A matrix of [X;Y;Z] ecliptic positions.
            %          - The rotation matrix for the conversion.
            % Author : Eran Ofek (May 2022)
            % Example: CooJ = celestial.INPOP.ecliptic_2eqJ2000(rand(3,6))
    ...
    methods % ephemeris evaluation
        function Pos = getPos(Obj, Object, JD, Args)
            ...
            %IndJD        = ceil((JD - Tstart(1))./Tstep);  % BUG / Chat
            ...
            ChebyEval    = Obj.ChebyFun{Norder}(JDTmid); % same for [x,y,z]
            for Icoo=1:1:Args.Ncoo
                ...
                %ChebyEval    = Obj.ChebyFun{Norder}(JDTmid); % same for
                %[x,y,z] so taken out of the loop
                
                %Pos(Icoo,:)  = sum([ChebyCoef.*ChebyEval(:,1:Norder)].',1);
                Pos(Icoo,:) = sum(ChebyCoef .* ChebyEval(:,1:Norder), 2).';  % faster
            end
