 ...
 otherwise
    ...
    switch lower(Direction)
     case {'curve_on_spec','cos'}
        ...
        if iscell(Spec)
            [Spec,Tran]     = astro.spec.eq_sampling(Spec,Tran,Spec{1},InterpMethod);
            if isvector(Spec{2})
                Spec{2}=Spec{2}(:);
            end
%             I = find(~isnan(Tran(:,2)));
...
%             Tran = Tran(I,:);
     ...
     case {'spec_on_curve','soc'}
        ...
        Spec     = astro.spec.eq_sampling(Spec,Tran,Tran(:,1));
     otherwise
