...
if (isempty(InPar.Dist))
    Dist = astro.cosmo.lum_dist(InPar.redshift);
else
    Dist = InPar.Dist.*1e6;  % [pc]
    InPar.redshift = astro.cosmo.inv_lum_dist(Dist);
end
...
if (nargout>3) && (~isempty(InPar.FiltFam) || ~isempty(InPar.Wave))
    ...
    if (isempty(InPar.Wave))
        ...
        Spec{1} = WaveRange;
        [~,~,Spec{2}] = astro.spec.black_body(redshiftedTc(:),WaveRange');
        ...
        if any(InPar.Ebv>0)
            Mag = astro.spec.synphot(Spec,Filter,[],InPar.FiltSys,[],InPar.Ebv,InPar.Rv,'photon');
        else
            Mag = astro.spec.synphot(Spec,Filter,[],InPar.FiltSys,[],[],[],'photon');
        end
    else 
        [~,In]=astro.spec.black_body(Tc./(1+InPar.redshift),InPar.Wave).*(1+InPar.redshift).^4;
        Mag = convert.flux(In.*R.^2./(Dist.*Pc).^2,'cgs/Hz','AB',InPar.Wave,'A');
        if any(InPar.Ebv>0)
            A = astro.extinction.extinction(InPar.Ebv,InPar.FiltFam,InPar.FiltName,InPar.Rv);
            Mag = Mag + A;
        end
    end
    Mag = reshape(Mag,S);
