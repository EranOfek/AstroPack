...
if (nargout>3)
    ...
    if (isempty(InPar.Wave))
        ...
        Mag = astro.spec.blackbody_mag_c(Tc./(1+InPar.redshift),InPar.FiltFam,InPar.FiltName,InPar.FiltSys,R,Dist);
        Mag = Mag - 2.5.*log10((1+InPar.redshift)^4);
    else
        Pc = get_constant('pc');
        [~,In]=astro.spec.black_body(Tc./(1+InPar.redshift),InPar.Wave);
