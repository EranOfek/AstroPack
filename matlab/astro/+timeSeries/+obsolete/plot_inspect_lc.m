...
[PS,Peaks] = timeSeries.period.period(LC,FreqVec,'Type',InPar.TypePS);
Ipeak = numel(Peaks.Freq);

PSw = timeSeries.period.period(LC,FreqVec,'Type','WinNL');
...
while Cont
    % folded LC
    FLC = timeSeries.fold.folding(LC,Peaks.Per(Ipeak));
    fprintf('Folded period: %f\n',Peaks.Per(Ipeak));
    
    B   = timeSeries.bin.binning(FLC,InPar.PhaseBinSize,[0 1],{'MidBin',@median,@std,@numel});
    B(:,3) = B(:,3)./sqrt(B(:,4));
