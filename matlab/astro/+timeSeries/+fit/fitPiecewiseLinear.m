function [Result, NullResult] = fitPiecewiseLinear(T, M, ErrorM, Args)
...
%
% Output : - NullResult : Structure array of size (1 x S).  Null-hypothesis
%                     fit — a single weighted constant (slope = 0) over all N
%                     points.  Used to quantify the improvement of the
%                     piecewise model via delta-chi^2 = NullResult.Chi2 - sum(Result.Chi2).
%            .Param    - [1 x 1] double. Weighted mean (the fitted constant).
%            .ParamErr - [1 x 1] double. 1-sigma error: 1/sqrt(sum_w).
%            .Chi2     - [1 x 1] double. chi^2 of the constant fit.
%                        chi^2 = sum_wm2 - sum_wm^2/sum_w  (>= 0).
%            .Dof      - [1 x 1] double. Degrees of freedom = N - 1.
%            .Npt      - [1 x 1] double. Number of data points = N.
%
...
%          disp(Res(1).Seg)   % slopes and intercepts for source 1
    ...
    %----------------------------------------------------------------------
    [T, SortIdx] = sort(T, 'ascend');   % MATLAB sort is stable by default
    M            = M(SortIdx, :);
    ErrFull      = ErrFull(SortIdx, :);

    %======================================================================
    % 1.  Pre-allocate output structure arrays
    %======================================================================
    ZeroSeg  = zeros(2, Ns);
    ZeroStat = zeros(1, Ns);
    ...
    EmptyNull  = struct('Param', 0, 'ParamErr', 0, 'Chi2', 0, 'Dof', 0, 'Npt', 0);
    NullResult = repmat(EmptyNull, 1, S);
    ...
    if Args.UseParallel
        parfor Si = 1:S
            [Result(Si), NullResult(Si)] = ...
                fitOneSource(T, M(:,Si), ErrFull(:,Si), N, Ns, Mp);
        end
    else
        for Si = 1:S
            [Result(Si), NullResult(Si)] = ...
...
%==========================================================================
function [Out, Null] = fitOneSource(T, Mv, ErrVec, N, Ns, Mp)
    ...
    %----------------------------------------------------------------------
    % Null hypothesis: fit a single weighted constant  m = c  over all N
    % points.  All quantities follow from the full-range prefix-sum values
    % (index N+1 of each array) — no extra loops or memory needed.
    %
    %   c        = sum_wm / sum_w               (weighted mean)
    %   sigma_c  = 1 / sqrt(sum_w)              (from Fisher information)
    %   chi^2    = sum_wm2 - sum_wm^2 / sum_w   (= sum_wm2 - c*sum_wm)
    ...
    NullParam    = NullSwM / NullSw;
    NullParamErr = 1 / sqrt(NullSw);
    NullChi2     = max(0, NullSwM2 - NullParam * NullSwM);

    Null = struct('Param',    NullParam,    ...
                  'ParamErr', NullParamErr, ...
                  'Chi2',     NullChi2,     ...
                  'Dof',      N - 1,        ...
                  'Npt',      N);
