...
%          [PS,Peaks]=timeSeries.period.period([X,Y]);
%          [PS,Peaks]=timeSeries.period.period([X,Y],[0 2 0.001]);
%          [PS,Peaks]=timeSeries.period.period([X,Y],[0 2 0.001],'Type','Scargle');
%          [PS,Peaks]=timeSeries.period.period([X,Y],[],'Taper','cosbell','TaperPar',0.5);
% Reliable: 2
%--------------------------------------------------------------------------
import timeSeries.period.*
...
switch lower(Par.Type)
 ...
 case 'wincomplex'
    Nd = size(Data,1);
    PS = period_complex([Data(:,1),ones(Nd,1)],Freq,'amp',false);  
 case 'win'
    Nd = size(Data,1);
