...
%          any bug/anomalies.
% Example: [P,F,T,Fr]=timeSeries.period.periodit([(1:1:1000)',rand(1000,1)],[30,0.1],[],[],'s');
% Tested : Matlab 5.1
...
if (PeriodType=='c'),
   [P,F] = timeSeries.period.periodia(TimeSerMat(1:ceil(0.1.*Nobs),:),LowFreq,HighFreq,FreqInt);
elseif (PeriodType=='s'),
   [P,F] = timeSeries.period.periodis(TimeSerMat(1:ceil(0.1.*Nobs),:),LowFreq,HighFreq,FreqInt);
else
...
for Ind=0:1:EpochNum-2,
   ...
   if (length(NewMat(1,:))==1),
      ...
      MidTime(1,Ind+1) = StartEpoch+Ind.*EpochStep + 0.5.*EpochLenT;
      % tapering
      [WeightFun,Range] = timeSeries.filter.cosbell(TaperFunP,NewMat(:,ColT));
      NewMat(:,ColO) = NewMat(:,ColO).*WeightFun;
      if (PeriodType=='c'),
         [P,F] = timeSeries.filter.periodia(NewMat,LowFreq,HighFreq,FreqInt);
      elseif (PeriodType=='s'),
         [P,F] = timeSeries.filter.periodis(NewMat,LowFreq,HighFreq,FreqInt);
      else
         error('unknown periodogram type');
      end
      SuperP(:,Ind+1) = P(:,2);
