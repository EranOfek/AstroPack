...
% Example: [B,C]=timeSeries.bin.binning_npt(rand(102,2))
%          [B,C]=timeSeries.bin.binning_npt(rand(102,2),30,{'medianbin',@range,@max})
% Reliable : 2
...
for Icol=1:1:Ncol
    ...
    if (ischar(ColCell{Icol}))
        switch lower(ColCell{Icol})
            case 'meanbin'
	        Bin(:,Icol) = mean(ReshapeTSt,1,'omitnan').';
            case 'medianbin'
                Bin(:,Icol) = median(ReshapeTSt,1,'omitnan').';
            case 'stdbin'
                Bin(:,Icol) = std(ReshapeTSt,[],1,'omitnan').';
            case 'rangebin'
