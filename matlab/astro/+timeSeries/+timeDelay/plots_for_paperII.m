...
%       'single'
%     
% Example: timeSeries.timeDelay.plots_for_paperII('single');
...
switch lower(Plot)
    case 'single'
        ...
        %[ResLC.F_t,Slope]=timeSeries.timeDelay.end_matching(ResLC.T,ResLC.F_t);
        %[ResLC.x_t,Slope]=timeSeries.timeDelay.end_matching(ResLC.T,ResLC.x_t);
        %%
        ...
        for Isim=1:1:Nsim
            [ResLC,ResG]=generateLC(InPar);
            [Res(Isim)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,ResLC.x_t,ResLC.y_t,ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
        ...
        f_t=timeSeries.timeDelay.reconstruct_ft(ResLC.F_t,ResLC.x_t,InPar.x0,InPar.x,InPar.A0,InPar.A(1),ResLC.sigma_F_hat,ResLC.sigma_x_hat);
        ...
        Ghat = fft(G);
        Fhat = fft(ResLC.F_t);
        w    = timeSeries.timeDelay.fft_freq(numel(ResLC.F_t),1);
    ...
    case 'singleexample'
        ...
        [Res(Isim,Irot)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                    'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                    'EndMatching',true);
        ...
        for Ia1=1:1:Na1
            ...
            for Ia2=1:1:Na2
                ...
                [LogL_xF(Ia1,Ia2),LogL_GF(Ia1,Ia2),LogL_F(Ia1,Ia2)]=timeSeries.timeDelay.logl_xF([VecA1(Ia1), A2],'FitPar',FitPar,...
                                                't',ResLC.T,...
                                                'F_t',ResLC.F_t,...
                                                'x_t',ResLC.x_t,...
    ...
    case 'fit_ax'
        ...
        for Isim=1:1:Nsim
            ...
            for Irot=1:1:Nrot
                ...
                [Res(Isim,Irot)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,...
                            sigma_x,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);
...
%         end
    ...
    case 'wronggamma'
        ...
        for Isim=1:1:Nsim
            ...
            for Igamma=1:1:Ngamma
                ...
                [Res(Isim,Igamma)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);
    ...
    case 'sens_a2'
        ...
        for Isim=1:1:Nsim
            ...
            for Ia2=1:1:Na2
                ...
                [Res(Isim,Ia2)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);
    ...
    case 'test_timed'
        ...
        for Itd=1:1:Ntd
            ...
            [Res(Itd)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                        'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                        'EndMatching',true);
        end
    ...
    case 'nonzero_a0_cont'
        ...
        freqs = timeSeries.timeDelay.fft_freq(N, t_step);
        ...
        for Ia0=1:1:Na0
            for Ia2=1:1:Na2
                ...
                [LogL_xF,LogL_GF,LogL_F]=timeSeries.timeDelay.logl_xF(Pars,...
                                         'FitPar',Pars,...
                                         'w',w,...
                                         'Limits',Limits,...
                                         't',t,...
    ...
    case 'nonzero_a0'
        ...
        for Isim=1:1:Nsim
            for Ia0=1:1:Na0
                ...
                [Res(Isim,Ia0)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);
                ...
                [ResW(Isim,Ia0)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);     
                        'a'
            end
    ...
    case 'sens_sigma_f'
        ...
        for Isim=1:1:Nsim
            ...
            for Is=1:1:Ns
                ...
                [Res(Isim,Is)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                            'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                            'EndMatching',true);
        ...
        for Isim=1:1:Nsim
            ...
            for Is=1:1:Ns
                ...
                for Islope=1:1:Nslope
                    ...
                    [Res(Isim,Is,Islope)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',ResLC.sigma_F_hat,ResLC.sigma_x_hat,...
                                'VecInvTau',VecInvTau,'DefPar',DefPar,'FitPar',FitPar,'Min_w',[0.0001 Inf],...
                                'EndMatching',true);
    ...
    case 'unevenly'
        ...
        for Isim=1:1:Nsim
            ...
            [Res(Isim)] = timeSeries.timeDelay.fit_astrometric_flux(ResLC.T,ResLC.F_t,XY(1,:).',XY(2,:).',...
    ...
    case 'alpha'
        ...
        Res=timeSeries.timeDelay.fit_scan_alpha_astrometric_flux(ResLC.T, ResLC.F_t, ResLC.x_t, ResLC.sigma_F_hat, ResLC.sigma_x_hat,...
    ...
    case 't_tau'
        ...
        for Isim=1:1:Nsim
            ...
            [Chi2_H0,Nobs,F_t,f_t] = timeSeries.timeDelay.chi2_xF(ParH0,FitParH0,'t',ResLC.T,'F_t',ResLC.F_t,'x_t',ResLC.x_t,'ErrF_t',ResLC.sigma_F_hat,'Errx_t',ResLC.sigma_x_hat);
            ...
            for Itau=1:1:Ntau
                ...
                [Chi2(Itau),Nobs,F_t,f_t] = timeSeries.timeDelay.chi2_xF(Par,FitPar,'t',ResLC.T,'F_t',ResLC.F_t,'x_t',ResLC.x_t,'ErrF_t',ResLC.sigma_F_hat,'Errx_t',ResLC.sigma_x_hat);
    ...
    case 't_a0a1a2'
        ...
        for Isim=1:1:Nsim
            ...
            Res(Isim)=timeSeries.timeDelay.fit_astrometric_flux_simple('FitPar',FitPar,'DefPar',DefPar,...
