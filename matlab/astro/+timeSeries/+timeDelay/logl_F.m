...
% Example: 
% [LogLikeF,Sigma_F,Sigma_phi]=timeSeries.timeDelay.logl_BPL([1 2],[NaN NaN 0 0],[],[],ResLC.w, ResLC.F_w, ResLC.sigma_F_hat)
    ...
    elseif ~isempty(InPar.F_t) && ~isempty(InPar.t)
        ...
        if size(CT,1)==size(CT,2)
            ...
            if numel(InPar.sigma_F)>1
                CT = timeSeries.timeDelay.equal_diagonals_matrix(CT);
                CT = CT + diag(InPar.sigma_F.^2);
            else
        ...
        LogLikeF = 0.5.*abs(timeSeries.timeDelay.logdet(2.*pi.*CT)) + 0.5.* (F_t.' * (CT\F_t));
        
        % it seems that G0 contribution is constant
%         V = logspace(3,5,100);
%         for K=1:1:numel(V)
%             DD = V(K);
%             LogLikeF(K) = 0.5.*abs(timeSeries.timeDelay.logdet(2.*pi.*CT)) + 0.5.* (F_t.' * ((CT+DD)\F_t));
%         end
