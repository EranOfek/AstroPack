...
% Example: [LogL_xF,LogL_GF,LogL_F]=timeSeries.timeDelay.logl_xF([1 0.5],...)
...
if Tau<Limits(1,1) || Tau>Limits(1,2) || ...
    ...
    if InPar.EndMatching
        [x_t,F_t] = timeSeries.timeDelay.end_matching_xt(t,x_t,F_t,x0,[x1 x2],Alpha0,[Alpha1,Alpha2],sigma_F_hat,sigma_x_hat);
        
        %[F_t,Slope,Diff]=timeSeries.timeDelay.end_matching(t,F_t);
        F_w = fft(F_t) ./ sqrt(N);
...
%         G_t_ = timeSeries.timeDelay.end_matching((1:1:N),G_t_);
%         G_w  = fft(G_t_)./sqrt(N);
%     end
...
%     InPar.EndMatching = false;
%     if InPar.EndMatching
%         F_tEM = timeSeries.timeDelay.end_matching(t,F_t);
...
%     else
    ...
    logdet_ = timeSeries.timeDelay.logdet(pi.*Gamma_G_given_F_);
    ...
    %[LogL_F]=timeSeries.timeDelay.logl_F(Pars, 'FitPar',FitPar,'Limits',Limits,'t',t,'F_t',F_t,'sigma_F',sigma_F_hat,'DT',(0:1:N-1))
