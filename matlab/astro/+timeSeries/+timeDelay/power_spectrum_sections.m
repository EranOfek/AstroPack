...
%              time series. Calculate the power spectrum of each section and
...
if isempty(Section)
    ...
    FreqVec = timeSeries.timeDelay.fft_freq(MinRange,1);
...
%         Section(Isec).FreqVec = ifftshift(timeSeries.timeDelay.fft_freq(Section(Isec).Nt)./Section(Isec).Nt);  % used to be TimeRange !!!
% 
%         Section(Isec).PS = fft(Section(Isec).Flux);
%     %     [FT,FreqVec,FT2] = timeSeries.timeDelay.fft_withphase(Section(Isec).Time,Section(Isec).Flux);
%     %     Section(Isec).PS = FT2;
%     %     
% 
