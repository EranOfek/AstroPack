...
% Example: [Res]=timeSeries.timeDelay.rand_lensed;
...
Freqs = timeSeries.timeDelay.fft_freq(Nt, InPar.DeltaT);
w = 2.*pi.*Freqs(:);
...
%[Sigma_phi,Sigma_F] = timeSeries.timeDelay.sigma_phi(w,Tau,A,Gamma,Epsilon_hat_F);
...
eps_F_w = timeSeries.timeDelay.rand_psd(Sigma_eps_);
f_red_w = timeSeries.timeDelay.rand_psd(Sigma_red_);
...
if ~InPar.Cyclic
    ...
    Freqs = timeSeries.timeDelay.fft_freq(IndEnd, InPar.DeltaT);
    Res.w = 2.*pi.*Freqs(:);
...
if AliasFactor~=1
    ...
    Nt = numel(Res.T);
    Freqs = timeSeries.timeDelay.fft_freq(Nt, InPar.DeltaT.*AliasFactor);
    Res.w = 2.*pi.*Freqs(:);
...
if InPar.Validate && (any(Res.F_t<0) || Res.StdDivMeanFlux<InPar.StdMeanRange(1) || Res.StdDivMeanFlux>InPar.StdMeanRange(2))
    ...
    [Res]=timeSeries.timeDelay.rand_lensed(varargin{:},'f_dc',InPar.f_dc./Factor);
end
...
if InPar.EndMatching
    ...
    % end matching
    [Res.F_t] = timeSeries.timeDelay.end_matching(Res.T,Res.F_t);
    
    N = numel(Res.F_t);
    Freqs = timeSeries.timeDelay.fft_freq(N, InPar.DeltaT.*AliasFactor);
    Res.w = 2.*pi.*Freqs(:);
    Res.F_w = fft(Res.F_t)./sqrt(N);
end
