...
W = io.files.load2('GAIA_Wave1A.mat');
...
for Ispec=1:1:Nspec
    Ind = Ind + 1;
    Spec = io.files.load2(Dir(Ispec).name);
    for Ifilt=1:1:Nfilt
        [Data(Ifilt,Ind).Mag,Data(Ind,Ifilt).Frac] = astro.spec.synphot([W, Spec],InPar.FilterList{Ifilt,1},InPar.FilterList{Ifilt,2},InPar.MagSystem);
...
for Ispec=1:1:Nspec
    ...
    for Iz=1:1:Nz
        ...
        for Ifilt=1:1:Nfilt
            [Data(Ifilt,Ind).Mag,Data(Ind,Ifilt).Frac] = astro.spec.synphot(Spec1,InPar.FilterList{Ifilt,1},InPar.FilterList{Ifilt,2},InPar.MagSystem);
            Data(Ifilt,Ind).SpecName = Dir(Ispec).name;
        end
    end
end
