...
// @Chen
CFITS_API const char** get_wcs_errmsg();
...
int CFITS_API wcsnpv(int n);

int CFITS_API wcsnps(int n);

int CFITS_API wcsini(int alloc, int naxis, struct wcsprm *wcs);

int CFITS_API wcsinit(int alloc, int naxis, struct wcsprm *wcs, int npvmax, int npsmax,
            int ndpmax);

int CFITS_API wcsauxi(int alloc, struct wcsprm *wcs);

int CFITS_API wcssub(int alloc, const struct wcsprm *wcssrc, int *nsub, int axes[],
           struct wcsprm *wcsdst);

int CFITS_API wcscompare(int cmp, double tol, const struct wcsprm *wcs1,
               const struct wcsprm *wcs2, int *equal);

int CFITS_API wcsfree(struct wcsprm *wcs);

int CFITS_API wcsprt(const struct wcsprm *wcs);

int CFITS_API wcsperr(const struct wcsprm *wcs, const char *prefix);

int CFITS_API wcsbchk(struct wcsprm *wcs, int bounds);

// @Chen - Conflict with Visual Studio, function wcsset() which is defined in header file
// C:\Program Files (x86)\Windows Kits\10\Include\10.0.18362.0\ucrt\corecrt_wstring.h
// Renamed wcsset to wcssetprm
int CFITS_API wcssetprm(struct wcsprm *wcs);

int CFITS_API wcsp2s(struct wcsprm *wcs, int ncoord, int nelem, const double pixcrd[],
           double imgcrd[], double phi[], double theta[], double world[],
           int stat[]);

int CFITS_API wcss2p(struct wcsprm *wcs, int ncoord, int nelem, const double world[],
           double phi[], double theta[], double imgcrd[], double pixcrd[],
           int stat[]);

int CFITS_API wcsmix(struct wcsprm *wcs, int mixpix, int mixcel, const double vspan[],
           double vstep, int viter, double world[], double phi[],
           double theta[], double imgcrd[], double pixcrd[]);

int CFITS_API wcssptr(struct wcsprm *wcs, int *i, char ctype[9]);
